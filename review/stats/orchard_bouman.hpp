...
private:
    ...
    {
        vector_weight_fun(const std::vector<real_type>& weights)
        : weights_(weights)
        {
        }
...
public:
    ...
    struct cluster_t
    {
        ...
        void compute_stats(const std::vector<vector_type>&      samples,
                           WeightFun                            wfun,
        ...
        template<class WeightFun>
        void split(cluster_t&                      other,
                   const std::vector<vector_type>& samples,
                   WeightFun                       wfun,
                   std::vector<const cluster_t*>&  labels)
        {
            RAMEN_ASSERT(!empty());
...
private:
    ...
    void do_cluster_samples(const std::vector<vector_type>& samples, WeightFun wfun)
    {
        ...
        clusters_.erase(std::remove_if(clusters_.begin(),
                                       clusters_.end(),
                                       boost::bind(&orchard_bouman_t::cluster_t::empty, _1)),
                        clusters_.end());
    }
