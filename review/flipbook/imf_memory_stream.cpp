...
namespace imageio
{
imf_memory_istream::imf_memory_istream(const char* data, std::size_t size)
: Imf::IStream("")
{
    data_ = data;
    size_ = size;
...
imf_memory_istream::imf_memory_istream(const imf_memory_istream& other)
: Imf::IStream("")
{
    data_ = other.data_;
    size_ = other.size_;
...
imf_memory_ostream::imf_memory_ostream(std::size_t mem_size)
: Imf::OStream("")
, buffer_(0)
{
    RAMEN_ASSERT(mem_size != 0 && "imf_memory_ostream: zero mem_size");
    mem_size_ = mem_size;
    pos_      = 0;
    ...
    {
        buffer_
            = reinterpret_cast<char*>(app().memory_manager().image_allocator().allocate(mem_size));
    }
    catch (std::bad_alloc&)
    {
        errno = ENOMEM;
...
{
    if (buffer_)
        app().memory_manager().image_allocator().deallocate(
            reinterpret_cast<unsigned char*>(buffer_), mem_size_);
}
...
{
    RAMEN_ASSERT(pos_ + n < mem_size_);
    memcpy((void*) &(buffer_[pos_]), (void*) c, n);
    pos_ += n;
}

// null ostream
imf_null_ostream::imf_null_ostream()
: Imf::OStream("")
, size_(0)
, pos_(0)
{
