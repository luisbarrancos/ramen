...
: QGLWidget()
, flipbook_t(frame_rate, display_device, display_transform)
{
...
internal_flipbook_t* internal_flipbook_t::create(int                frame_rate,
                                                 const std::string& display_device,
                                                 const std::string& display_transform)
{
...
void internal_flipbook_t::write_exr(Imf::OStream&                      os,
                                    Imf::Header&                       header,
                                    const boost::gil::rgba16fc_view_t& view) const
{
...
{
    QMessageBox::warning((QWidget*) app().ui()->main_window(), "OpenGL error", message);
    exit(0);
}
...
Imath::V2f internal_flipbook_t::screen_to_world(const Imath::V2i& p) const
{
    return viewport_.screen_to_world(p);
}
Imath::V2i internal_flipbook_t::world_to_screen(const Imath::V2f& p) const
{
...
void internal_flipbook_t::update_texture()
{
    gl_enable(GL_TEXTURE_2D);
    gl_bind_texture(GL_TEXTURE_2D, texture_id_);
    gl_tex_subimage2d(GL_TEXTURE_2D,
                      0,
                      0,
                      0,
                      image_width(),
                      ...
                      GL_RGBA,
                      GL_HALF_FLOAT_ARB,
                      (GLvoid*) ib_.pixels(buffer_index_, 0));
    gl_disable(GL_TEXTURE_2D);
}
