...
void display(void)
{
    ...
    switch (disp)
    {
        ...
        case display_tri:
            glDrawPixels(
                image.cols(), image.rows(), GL_LUMINANCE, GL_UNSIGNED_BYTE, trimap.dataFirst());
            break;
    }
...
{
    vigra::BasicImageView<vigra::TinyVector<float, 3>> img_view(
        (vigra::TinyVector<float, 3>*) img.dataFirst(), img.cols(), img.rows());

    vigra::transformImage(vigra::srcImageRange(img_view),
                          vigra::destImage(img_view),
                          vigra::RGB2LabFunctor<float>(1.0f));
...
void convert2rgb(blitz::Array<Imath::Color3f, 2>& img)
{
    ...
    vigra::transformImage(vigra::srcImageRange(img_view),
                          vigra::destImage(img_view),
                          vigra::Lab2RGBFunctor<float>(1.0f));
}
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        desc.add_options()("help", "produce help message")
            ...
            // gui
            ("window,w", "show results in a window");  // end
