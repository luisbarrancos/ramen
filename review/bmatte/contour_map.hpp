...
public:
    cm_cell(Imath::V2i p, int cell_size = 64)
    : c(p)
    , csize(cell_size)
    {
    }
    ...
    void nearest_point(Imath::V2i p, Imath::V2i& np, int& d)
    {
        ...
        if (!points.empty())
        {
            ...
            {
                dist = ((p.x - points[k].x) * (p.x - points[k].x))
                       + ((p.y - points[k].y) * (p.y - points[k].y));
...
public:
    ...
    int nearest_point(Imath::V2i p, Imath::V2i& np)
    {
        ...
        while (window_size < max_window)
        {
            ...
            {
                adj_left = 1;
                for (int s = std::max(i - window_size, 0); s < std::min(i + window_size + 1, nx);
                     s++)
            ...
            if (i - window_size >= 0)
                for (int s = std::max(j - window_size + adj_left, 0);
                     s < std::min(j + window_size + 1 - adj_right, ny);
                     s++)
