...
bmatte::bmatte(const blitz::Array<Imath::Color3f, 2>& image,
               const blitz::Array<uint8_t, 2>         tri,
               blitz::Array<Imath::Color3f, 2>        fg,
               blitz::Array<Imath::Color3f, 2>        bg,
...
void bmatte::build_dmap(uint8_t val, distance_map& dmap)
{
    for (int j = 1; j < mask_.height() - 1; ++j)
    {
        ...
        {
            if (mask_(j, i) == val)
                if (mask_(j - 1, i) != val || mask_(j + 1, i) != val || mask_(j, i - 1) != val
                    || mask_(j, i + 1) != val)
    ...
    for (int j = ymin; j < ymax; ++j)
    {
        for (int i = xmin; i < xmax; ++i)
        {
            if (computed_(j, i))
            {
                ...
                {
                    col = bg_(j, i);
                    bg_cluster_.add_sample(color_type(col.x, col.y, col.z),
                                           w * (1.0 - a) * (1.0 - a));
                }
            }
        }
...
{
    color_cluster_type::cluster_iterator it(fg_cluster_.clusters_begin()),
        it2(bg_cluster_.clusters_begin());
...
float bmatte::optimize_pair(bmatte::color_cluster_type::cluster_iterator it,
                            bmatte::color_cluster_type::cluster_iterator it2,
                            const color_type&                            c,
                            color_type&                                  fg,
                            color_type&                                  bg,
                            double&                                      alpha)
{
    ...
    return lc(c, fg, bg, a) + lf(fg, it->mean(), it->inverse_covariance())
           + lb(bg, it2->mean(), it2->inverse_covariance());
}
