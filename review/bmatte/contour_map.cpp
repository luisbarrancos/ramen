...
void cm_cell::nearest_point(int x, int y, int* nx, int* ny, int* d)
{
    ...
    if (!points.empty())
    {
        ...
        {
            dist
                = ((x - points[k].x) * (x - points[k].x)) + ((y - points[k].y) * (y - points[k].y));
...
int contour_map::nearest_point(int x, int y, int* xn, int* yn)
{
    ...
    while (window_size < max_window)
    {
        ...
        if (i - window_size >= 0)
            for (int s = max(j - window_size + adj_left, 0);
                 s < min(j + window_size + 1 - adj_right, ny);
                 s++)
        ...
        case GAN_UCHAR:
            ...
            for (unsigned int j = 0; j < mask->height; j++)
            {
                ...
                for (unsigned int i = 0; i < mask->width; i++)
                {
                    if (*p1 == v)
                    {
                        if ((i != 0 && *(p1 - 1) != v) || (i != mask->width - 1 && (*(p1 + 1) != v))
                            || (j != 0 && *p0 != v) || (j != mask->height - 1 && *p2 != v))
                        {
                            s = i / map->csize;
                            t = j / map->csize;
