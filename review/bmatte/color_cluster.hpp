...
// lapack eigenvalues
template<class X>
long xspev(X* symm, int size, X* eigvals, X* eigvecs, X* work);
template<>
long xspev(float* symm, int size, float* eigvals, float* eigvecs, float* work);
template<>
long xspev(double* symm, int size, double* eigvals, double* eigvecs, double* work);
...
public:
    ...
    {
    public:
        sample(const Imath::Color3<T>& col, T weight = T(1))
        : c(col)
        , w(weight)
        {
        }
    ...
    {
        cluster_select(const cluster* c)
        : c_(c)
        {
        }
    ...
    public:
        ...
        // for std::max_element
        bool operator<(const cluster& c) const { return max_eigval_ < c.max_eigval_; }

        void calc_stats(const_sample_iterator first, const_sample_iterator last)
        {
            boost::filter_iterator<cluster_select, const_sample_iterator> it(
                cluster_select(this), first, last);
            boost::filter_iterator<cluster_select, const_sample_iterator> l(
                cluster_select(this), last, last);
            ...
            if (num_samples_ > 1)
            {
                it = boost::filter_iterator<cluster_select, const_sample_iterator>(
                    cluster_select(this), first, last);
            ...
            {
                cov_ = Imath::Matrix33<T>(min_var_, 0, 0, 0, min_var_, 0, 0, 0, min_var_);

                eigvals_    = Imath::Vec3<T>(min_var_, min_var_, min_var_);
                eigvecs_    = Imath::Matrix33<T>(1, 0, 0, 0, 1, 0, 0, 0, 1);
        ...
        {
            boost::filter_iterator<cluster_select, sample_iterator> it(
                cluster_select(this), first, last);
            boost::filter_iterator<cluster_select, sample_iterator> l(
                cluster_select(this), last, last);
            Imath::Vec3<T> axis;
    ...
    color_cluster(int max_clusters, T max_var)
