...
{
    flow_warp_fun(const image::const_image_view_t& src,
                  const Imath::Box2i&              src_area,
                  const image::const_image_view_t& flow,
                  const Imath::Box2i&              flow_area,
                  int                              flow_ch_offset,
                  const Imath::V2f&                flow_scale,
                  const image::image_view_t&       dst,
                  const Imath::Box2i&              dst_area)
    : src_sampler_(src_area, src)
    , flow_sampler_(flow_area, flow)
    , dst_(dst)
    {
    ...
    inline image::pixel_t operator()(const Imath::V2d& p) const
    {
        ...
        Imath::V2d     uv(px[flow_channel_offset_] * flow_scale_.x,
                      px[flow_channel_offset_ + 1] * flow_scale_.y);
...
{
    flow_warp_mix_fun(const image::const_image_view_t& src,
                      const Imath::Box2i&              src_area,
                      const image::const_image_view_t& flow,
                      const Imath::Box2i&              flow_area,
                      int                              flow_ch_offset,
                      ...
                      const Imath::Box2i&              dst_area,
                      float                            mix)
    : flow_warp_(src, src_area, flow, flow_area, flow_ch_offset, flow_scale, dst, dst_area)
    , dst_(dst)
    {
        dst_area_ = dst_area;
        mix_      = mix;
...
retime_node_t::retime_node_t()
: image_node_t()
{
    add_input_plug("front", false, ui::palette_t::instance().color("front plug"), "Front");
    add_input_plug(
        "mvectors", true, ui::palette_t::instance().color("back plug"), "Motion Vectors");
    add_output_plug();
    set_name("retime");
}

retime_node_t::retime_node_t(const retime_node_t& other)
: image_node_t(other)
{
}
...
bool retime_node_t::do_is_identity() const { return get_src_frame() == composition()->frame(); }
...
void retime_node_t::do_retime(const render::context_t& context)
{
    ...
    if (interp == interp_motion)
    {
        ...
        tbb::parallel_for(tbb::blocked_range<int>(defined().min.y, defined().max.y + 1),
                          flow_warp_fun<sse2::bilinear_sampler_t>(pixels.const_rgba_view(),
                                                                  pixels.bounds(),
                                                                  flow.const_rgba_view(),
                                                                  flow.bounds(),
                                                                  0,
                                                                  flow_scale,
                                                                  image_view(),
        ...
        if (get_value<bool>(param("bidirect")))
        {
            ...
            tbb::parallel_for(tbb::blocked_range<int>(defined().min.y, defined().max.y + 1),
                              flow_warp_mix_fun<sse2::bilinear_sampler_t>(pixels.const_rgba_view(),
                                                                          pixels.bounds(),
                                                                          flow.const_rgba_view(),
                                                                          flow.bounds(),
                                                                          2,
    ...
    if (!area.isEmpty())
        image::lerp_images(const_subimage_view(area),
                           pixels.const_rgba_subimage_view(area),
                           1.0f - fract,
                           subimage_view(area));
}
...
static bool registered
    = node_factory_t::instance().register_node(retime_node_t::retime_node_metaclass());
