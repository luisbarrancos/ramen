...
tracker_areas_t::tracker_areas_t(const Imath::Box2f& ref, const Imath::Box2f& srch)
{
    set_areas(ref, srch);
}
...
{
    out << YAML::Flow << YAML::BeginSeq << (int) areas.reference.min.x
        << (int) areas.reference.min.y << (int) areas.reference.max.x << (int) areas.reference.max.y
        << (int) areas.search.min.x << (int) areas.search.min.y << (int) areas.search.max.x
        << (int) areas.search.max.y << YAML::EndSeq;
    return out;
}

tracker_areas_param_t::tracker_areas_param_t()
: static_param_t("Tracker Areas")
{
...
tracker_areas_param_t::tracker_areas_param_t(const tracker_areas_param_t& other)
