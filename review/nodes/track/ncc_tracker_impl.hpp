...
public:
    ...
    void update_reference(const Imath::Box2f& area, const image::buffer_t& pixels)
    {
        ...
        if (reference_.width() != ref_width || reference_.height() != ref_height
    ...
    Imath::V2f transform_ncc_match(float             x,
                                   float             y,
                                   const Imath::V2f& ref_pos,
                                   const Imath::V2i& ref_offset) const
    {
    ...
    Imath::V2f track()
    {
        ...
        tbb::parallel_for(
            tbb::blocked_range<int>(0, search_h),
            boost::bind(
                &ncc_tracker_t::impl::do_track, this, _1, ref_width, ref_height, ref_offset));
        ...
        if (confidence_ < 0.99f)
        {
            ...
            tbb::parallel_for(tbb::blocked_range<int>(0, 2 * (subpixel_res() + 1) + 1),
                              boost::bind(&ncc_tracker_t::impl::do_subpixel_track,
                                          this,
                                          _1,
                                          best_int_ncc,
                                          subpixel_res(),
            ...
            Imath::V2f best_subp_pos
                = subpixel_transform(find_best_ncc_match(subpixel_ncc_result_, subp_confidence))
                  + best_int_ncc;
...
private:
    ...
    {
        float search_mean
            = calc_mean(search_, search_x, search_y, search_x + ref_width, search_y + ref_height);
    ...
    float subpixel_ncc(float             search_x,
                       float             search_y,
                       float             ref_mean,
                       float             subpixel_inc,
                       int               ref_width,
                       ...
                       const Imath::V2i& ref_offset) const
    {
        if (search_x < 0 || search_y < 0 || search_x + ref_width >= search_.width()
            || search_y + ref_height >= search_.height())
            return 0;

        float num = 0, ref_denom = 0, search_denom = 0, denom = 0;

        float search_mean = calc_subpixel_mean(
            search_, search_x, search_y, search_x + ref_width, search_y + ref_height);
        ...
        for (float j = 0; j < ref_height; j += subpixel_inc)
        {
            ...
            {
                float ref_diff
                    = reference_.linear_atXY(ref_offset.x + i, ref_offset.y + j) - ref_mean;
    ...
    void do_subpixel_track(const tbb::blocked_range<int>& range,
                           ...
                           const Imath::V2i               ref_offset)
    {
        ...
        for (int y = range.begin(); y < range.end(); ++y)
        {
            ...
            for (int x = 0, xe = 2 * (subpixel_res + 1); x <= xe; ++x)
            {
                ...
                subpixel_ncc_result_(x, y) = subpixel_ncc(best_int_ncc.x + i,
                                                          best_int_ncc.y + j,
                                                          ref_mean_,
                                                          subpixel_inc,
                                                          ref_width,
    ...
    Imath::V2f subpixel_transform(const Imath::V2i& p) const
    {
        ...
        return Imath::V2f(((p.x / size) - 0.5f) * 2.0f, ((p.y / size) - 0.5f) * 2.0f);
    }
