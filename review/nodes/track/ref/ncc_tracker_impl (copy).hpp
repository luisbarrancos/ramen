...
public:
    impl(float adapt_tolerance)
    {
        ...
        subpixel_ncc_result_ = CImg<>(subpixel_res() * 3, subpixel_res() * 3);
    }
    ...
    {
        ref_pos_ = Imath::V2i(Imath::Math<float>::floor(p.x), Imath::Math<float>::floor(p.y));
    }

    void update_reference(const Imath::Box2f& area, const image::buffer_t& pixels)
    {
        Imath::Box2i ref_area = Imath::Box2i(
            Imath::V2i(Imath::Math<float>::floor(area.min.x),
                       Imath::Math<float>::floor(area.min.y)),
            Imath::V2i(Imath::Math<float>::ceil(area.max.x), Imath::Math<float>::ceil(area.max.y)));
        ...
        if (reference_.width() != ref_width_ || reference_.height() != ref_height_
            || reference_.spectrum() != 1)
    ...
    {
        Imath::Box2i search_area = Imath::Box2i(
            Imath::V2i(Imath::Math<float>::floor(area.min.x),
                       Imath::Math<float>::floor(area.min.y)),
            Imath::V2i(Imath::Math<float>::ceil(area.max.x), Imath::Math<float>::ceil(area.max.y)));
        ...
        if (search_.width() != search_width_ || search_.height() != search_height_
            || search_.spectrum() != 2)
            search_ = CImg<>(search_width_, search_height_, 1, 2, 0);

        do_copy_area(
            pixels, search_pos_.x - 1, search_pos_.y - 1, search_width_, search_height_, search_);
    ...
    Imath::V2f track()
    {
        ...
        if (ncc_result_.width() != search_w || ncc_result_.height() != search_h
            || ncc_result_.spectrum() != 1)
            ncc_result_ = CImg<>(search_w, search_h);

        // int pixel search
        tbb::parallel_for(tbb::blocked_range<int>(0, search_h),
                          boost::bind(&ncc_tracker_t::impl::do_track, this, _1));
        Imath::V2i best_int_ncc = find_best_ncc_match(ncc_result_, ncc_coeff_);

        // subpixel search
        /*
        subpixel_track_reduction_fun tf( this, best_int_ncc, subpixel_res());
        tbb::parallel_reduce( tbb::blocked_range<int>( 0, subpixel_res()), tf);
        ncc_coeff_ = tf.ncc_coeff;
        Imath::V2f max_ncc = tf.pos;
        */
        ...
        return transform_ncc_match(max_ncc.x, max_ncc.y);
    }
...
private:
    ...
    void make_integral_image(CImg<>& I)
    {
        ...
        for (int j = 1; j < h; ++j)
            for (int i = 1; i < w; ++i)
                I(i, j, 0, 1)
                    = I(i, j, 0, 0) + I(i - 1, j, 0, 1) + I(i, j - 1, 0, 1) - I(i - 1, j - 1, 0, 1);
    }
    ...
    float calc_subpixel_mean(
        const CImg<>& table, float xmin, float ymin, float xmax, float ymax) const
    {
        RAMEN_ASSERT(table.width() != 0 && table.height() != 0);
        RAMEN_ASSERT(table.spectrum() == 2);
        RAMEN_ASSERT(xmin >= 0 && xmin < table.width() && ymin >= 0 && ymin < table.height());
    ...
    {
        float search_mean = calc_mean(search_,
                                      search_x + search_offset_.x,
        ...
        for (int j = 0; j < ref_height_; ++j)
        {
            for (int i = 0; i < ref_width_; ++i)
            {
                ...
                float s = search_(search_x + i + search_offset_.x, search_y + j + search_offset_.y);
    ...
    float subpixel_ncc(float search_x, float search_y, float ref_mean, int subpixel_res) const
    {
        ...
        float search_mean = calc_subpixel_mean(search_,
                                               search_x + search_offset_.x,
                                               search_y + search_offset_.y,
                                               search_x + search_offset_.x + ref_width_,
                                               search_y + search_offset_.y + ref_height_);
        ...
        for (float j = 0; j < ref_height_; j += inc)
        {
            ...
            {
                float ref_diff
                    = reference_.linear_atXY(ref_offset_.x + i, ref_offset_.y + j) - ref_mean;
    ...
    Imath::V2f do_track(const tbb::blocked_range<int>& range)
    {
    ...
    Imath::V2f do_subixel_track(const tbb::blocked_range<int>& range,
                                const Imath::V2i&              best_int_ncc,
                                int                            subpixel_res,
                                float&                         ncc_result)
    {
        ...
        for (int y = range.begin(); y < range.end(); ++y)
        {
            ...
            for (int x = 0; x < subpixel_res; ++x)
            {
                ...
                float c
                    = subpixel_ncc(best_int_ncc.x + i, best_int_ncc.y + j, ref_mean_, subpixel_res);
    ...
    struct subpixel_track_reduction_fun
    {
        ...
        {
            pos = pimpl_->do_subixel_track(range, best_int_pos, subpixel_res, ncc_coeff);
        }
