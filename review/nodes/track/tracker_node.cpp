...
tracker_node_t::tracker_node_t(const tracker_node_t& other)
: image_node_t(other)
, tracking_(false)
{
    create_trackers();
...
void tracker_node_t::do_connected(node_t* src, int port)
{
    ...
    if (src)
        input_connection_
            = src->changed.connect(boost::bind(&tracker_node_t::input_changed, this, _1));
}
...
{
    float tracker_positions[] = { 177, 121, 377, 121, 377, 331, 177, 331 };

    float tracker_colors[] = { 1, 0, 0, 0, 1, 0, 0, 0, 1, 1, 0, 1 };
    ...
    for (int i = 0; i < num_trackers(); ++i)
    {
        ...
        c->set_is_rgba(false);
        c->set_static(true);
        c->set_default_value(Imath::Color4f(
            tracker_colors[i * 3], tracker_colors[i * 3 + 1], tracker_colors[i * 3 + 2], 1.0f));
        g->add_param(c);
...
void tracker_node_t::create_trackers()
{
    ...
    for (int i = 0; i < num_trackers(); ++i)
    {
        ...
        float2_param_t* track
            = dynamic_cast<float2_param_t*>(&param(std::string("track") + suffix));
        float2_param_t* offset
            = dynamic_cast<float2_param_t*>(&param(std::string("offset") + suffix));
        tracker_areas_param_t* areas
            = dynamic_cast<tracker_areas_param_t*>(&param(std::string("area") + suffix));
        float_param_t* adapt_tol = dynamic_cast<float_param_t*>(&param("adapt_tol"));
        float_param_t* conf
            = dynamic_cast<float_param_t*>(&param(std::string("confidence") + suffix));

        track::ncc_tracker_t* t
            = new track::ncc_tracker_t(active, color, track, offset, areas, adapt_tol, conf);
...
{
    Imath::M33f rot_v0_x(v0.x, -v0.y, 0, v0.y, v0.x, 0, 0, 0, 1);
...
void tracker_node_t::apply_track(float             start_frame,
                                 float             end_frame,
                                 apply_track_mode  mode,
                                 apply_track_use   use,
                                 const Imath::V2f& center,
                                 float2_param_t*   trans,
                                 float_param_t*    rot,
                                 float2_param_t*   scale) const
{
    // preconditions
    RAMEN_ASSERT(trans);
...
static bool registered
    = node_factory_t::instance().register_node(tracker_node_t::tracker_node_metaclass());
