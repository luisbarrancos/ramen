...
{
template<class Fun>
Fun for_each_point_in_path(const Imath::V2f&          default_value,
                           const anim::float_curve_t& xcurve,
                           const anim::float_curve_t& ycurve,
                           Fun                        f)
{
...
{
    pick_point_in_path(const Imath::V2f& pos, float pixel_scale)
    : picked(false)
    {
        pos_         = pos;
...
tracker_manipulator_t::tracker_manipulator_t()
: manipulator_t()
{
    picked_tracker_ = -1;
    mode_           = no_drag;
...
void tracker_manipulator_t::do_mouse_drag_event(const ui::mouse_drag_event_t& event)
{
    ...
    switch (mode_)
    {
        ...
        case search_drag:
        {
            ...
            if (mode_ == ref_drag)
                tracker_node()->trackers()[picked_tracker_]->move_reference_corner(
                    picked_corner_, offset, symmetric);
            else
                tracker_node()->trackers()[picked_tracker_]->move_search_corner(
                    picked_corner_, offset, symmetric);
        }
        break;
        ...
        case track_key_drag:
            ...
            tracker_node()->set_frame(tracker_node()->composition()->frame());  // <<-- TODO: check
                                                                                // this. Fix bugs in
                                                                                // floatX_param_t.
            break;
    };
...
void tracker_manipulator_t::draw_tracker(const track::ncc_tracker_t* t,
                                         const ui::paint_event_t&    event) const
{
    if (t->active())
    {
...
void tracker_manipulator_t::draw_tracker_path(const track::ncc_tracker_t* t,
                                              const ui::paint_event_t&    event,
                                              const Imath::Color3c&       color) const
{
    Imath::V2f                 pos(t->track_pos());
    ...
    for_each_point_in_path(
        pos, xcurve, ycurve, boost::bind(put_gl_vertex, _1, _2, _3, event.aspect_ratio));
    gl_end();
...
bool tracker_manipulator_t::pick_tracker(const track::ncc_tracker_t* t,
