...
namespace
{
Imath::Box2f drag_box_corner(const Imath::Box2f& b,
                             int                 corner,
                             const Imath::V2f&   d,
...
Imath::V2f ncc_tracker_t::track_pos(float frame) const
{
    return get_value_at_frame<Imath::V2f>(*track_, frame);
}
...
{
    return track_x_curve().has_keyframe_at(frame) || track_y_curve().has_keyframe_at(frame);
}
