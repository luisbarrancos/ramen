...
void makecolorwheel()
{
    ...
    int k = 0;
    for (i = 0; i < RY; i++)
        setcols(1.0f, (float) i / RY, 0.0f, k++);
    for (i = 0; i < YG; i++)
        setcols(1.0f - (float) i / YG, 1.0f, 0.0f, k++);
    for (i = 0; i < GC; i++)
        setcols(1.0f, 1.0f, (float) i / GC, k++);
    for (i = 0; i < CB; i++)
        setcols(1.0f, 1.0f - (float) i / CB, 1.0f, k++);
    for (i = 0; i < BM; i++)
        setcols((float) i / BM, 0.0f, 1.0f, k++);
    for (i = 0; i < MR; i++)
        setcols(1.0f, 0.0f, 1.0f - (float) i / MR, k++);
}
...
struct color_flow
{
    color_flow(bool backwards = false)
...
vis_flow_node_t::vis_flow_node_t()
: pointop_node_t()
{
    set_name("vis_flow");
}

void vis_flow_node_t::do_process(const image::const_image_view_t& src,
...
static bool registered
    = node_factory_t::instance().register_node(vis_flow_node_t::vis_flow_node_metaclass());
#endif
