...
{
    apply_lut_fun(const half_lut_t& lut)
    : lut_(lut)
    {
    }
...
{
    cineon_log_to_linear_fun(
        const IECore::CineonToLinearDataConversion<boost::uint16_t, float>& conv)
    : conv_(conv)
    {
    }
    ...
    image::pixel_t operator()(const image::pixel_t& src) const
    {
        ...
        boost::uint16_t glog
            = adobe::clamp((float) get_color(src, green_t()), 0.0f, 1.0f) * 1023.0f;
...
{
    cineon_linear_to_log_fun(
        const IECore::LinearToCineonDataConversion<float, boost::uint16_t>& conv)
    : conv_(conv)
    {
    }
    ...
    image::pixel_t operator()(const image::pixel_t& src) const
    {
        ...
        return image::pixel_t(conv_(rlin) / 1023.0f,
                              conv_(glin) / 1023.0f,
                              conv_(blin) / 1023.0f,
...
{
    cineon_linear_to_log_lut_fun(boost::uint16_t* lut)
    : lut_(lut)
    {
    }
...
log_to_linear_node_t::log_to_linear_node_t()
: pointop_node_t()
{
    set_name("log2lin");
}
...
void log_to_linear_node_t::do_process(const image::const_image_view_t& src,
                                      ...
                                      const render::context_t&         context)
{
    if (get_value<int>(param("convert")))
    {
        switch (get_value<int>(param("method")))
        {
            case cineon_conv:
            {
                IECore::LinearToCineonDataConversion<float, boost::uint16_t> conv(
                    get_value<float>(param("cin_gamma")),
            ...
            case ocio_conv:
            {
                ...
                try
                {
                    ...
                    OCIO::ConstProcessorRcPtr proc
                        = config->getProcessor(OCIO::ROLE_SCENE_LINEAR, OCIO::ROLE_COMPOSITING_LOG);
    ...
    else
    {
        switch (get_value<int>(param("method")))
        {
            case cineon_conv:
            {
                IECore::CineonToLinearDataConversion<boost::uint16_t, float> conv(
                    get_value<float>(param("cin_gamma")),
                    get_value<float>(param("cin_white")),
                    get_value<float>(param("cin_black")));
...
void log_to_linear_node_t::init_lut(
    boost::uint16_t*                                                    lut,
...
const node_metaclass_t* log_to_linear_node_t::metaclass() const
{
    return &log_to_linear_node_metaclass();
}
...
static bool registered = node_factory_t::instance().register_node(
    log_to_linear_node_t::log_to_linear_node_metaclass());
