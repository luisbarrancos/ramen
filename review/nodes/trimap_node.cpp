...
{
    thereshold_alpha_fun(float lo, float hi)
    : alpha_low_(lo)
    , alpha_high_(hi)
    {
    }
...
void set_unknown(image::pixel_t& p) { boost::gil::get_color(p, boost::gil::alpha_t()) = 0.5f; }

}  // unnamed

trimap_node_t::trimap_node_t()
: areaop_node_t()
{
    set_name("trimap");
}
...
void trimap_node_t::grow_trimap(std::vector<Imath::V2i>& pts)
{
    ...
    for (int y = 1; y < view.height() - 1; ++y)
    {
        for (int x = 1; x < view.width() - 1; ++x)
        {
            ...
            {
                if (is_unknown(src_loc[above]) || is_unknown(src_loc[below])
                    || is_unknown(src_loc[left]) || is_unknown(src_loc[right]))
...
static bool registered
    = node_factory_t::instance().register_node(trimap_node_t::trimap_node_metaclass());
#endif
