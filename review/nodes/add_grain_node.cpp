...
{
    apply_grain_fun(const image::const_image_view_t& src_view,
                    const image::image_view_t&       dst_view,
                    const noise::simplex_noise_t&    ngen,
                    const Imath::Box2i&              area,
                    const Imath::V3f&                freq,
                    float                            aspect,
                    const Imath::V3f                 intensity,
                    const Imath::V3f                 channel_offset,
                    float                            lo,
                    float                            mid,
                    float                            hi,
                    float                            soft,
    ...
    void operator()(const tbb::blocked_range<int>& r) const
    {
        for (int j = r.begin(); j != r.end(); ++j)
        {
            ...
            for (int i = area_.min.x; i <= area_.max.x; ++i)
            {
                ...
                amount
                    *= (lo_intensity_ * lo_w) + (mid_intensity_ * mid_w) + (hi_intensity_ * hi_w);
                ...
                *dst_it = image::pixel_t(std::max(rr, 0.0f),
                                         std::max(rg, 0.0f),
                                         std::max(rb, 0.0f),
...
add_grain_node_t::add_grain_node_t()
: pointop_node_t()
{
    set_name("add_grain");
...
void add_grain_node_t::do_process(const image::const_image_view_t& src,
                                  ...
                                  const render::context_t&         context)
{
    ...
    grain_size
        *= get_value<float>(param("grain_freq")) * 0.1f * context.subsample / proxy_scale().x;

    float aspect = get_value<float>(param("grain_aspect")) / aspect_ratio();

    apply_grain_fun f(src_view,
                      dst_view,
                      noise::global_noise,
                      area,
                      grain_size,
    ...
    tbb::parallel_for(
        tbb::blocked_range<int>(area.min.y, area.max.y + 1), f, tbb::auto_partitioner());
...
Imath::V3f add_grain_node_t::channel_offset(const render::context_t& context) const
{
    ...
    return Imath::V3f(rng(0.77f, context.frame, seed) / context.subsample / aspect_ratio() * 10,
                      ...
                      rng(21.62f, context.frame, seed) / context.subsample)
           * 10;
}
...
static bool registered
    = node_factory_t::instance().register_node(add_grain_node_t::add_grain_node_metaclass());
