...
namespace imageio
{
cineon_reader_t::cineon_reader_t(const boost::filesystem::path& p)
: reader_t(p)
{
    ...
    info_[adobe::name_t("format")]
        = adobe::any_regular_t(Imath::Box2i(Imath::V2i(0, 0), Imath::V2i(width_ - 1, height_ - 1)));
}

void cineon_reader_t::do_read_image(const image::image_view_t& view,
...
void cineon_reader_t::read_header(boost::filesystem::ifstream& in) const
{
    ...
    {
        CineonImageInformationChannelInformation& channelInformation
            = image_info.channel_information[i];

        if (reverse_bytes_)
        {
            channelInformation.pixels_per_line
                = IECore::reverseBytes(channelInformation.pixels_per_line);
            channelInformation.lines_per_image
                = IECore::reverseBytes(channelInformation.lines_per_image);
        }
        ...
        {
            if (channelInformation.pixels_per_line != width_
                || channelInformation.lines_per_image != height_)
