...
protected:
    ...
    // features
    void detect_surf_features(const cv::Mat&             img,
                              const cv::Mat&             msk,
    ...
    // descriptors
    void extract_surf_descriptors(const cv::Mat&             img,
                                  std::vector<cv::KeyPoint>& feat,
                                  cv::Mat&                   descriptors);
    ...
    // align
    boost::optional<Imath::M33d> find_similarity(const std::vector<cv::Point2d>& from_pts,
                                                 const std::vector<cv::Point2d>& to_pts);
    boost::optional<Imath::M33d> find_homography(const std::vector<cv::Point2d>& from_pts,
                                                 const std::vector<cv::Point2d>& to_pts);

    // rendering
    void get_frame(const render::context_t& context,
                   cv::Mat&                 dst,
                   cv::Mat&                 mask,
                   int                      use_alpha = 0);
