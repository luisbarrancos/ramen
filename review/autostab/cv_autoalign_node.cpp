...
cv_autoalign_node_t::cv_autoalign_node_t()
: autoalign_base_node_t()
{
}
cv_autoalign_node_t::cv_autoalign_node_t(const cv_autoalign_node_t& other)
: autoalign_base_node_t(other)
{
}
...
// features
void cv_autoalign_node_t::detect_surf_features(const cv::Mat&             img,
                                               const cv::Mat&             msk,
...
// descriptors
void cv_autoalign_node_t::extract_surf_descriptors(const cv::Mat&             img,
                                                   std::vector<cv::KeyPoint>& feat,
                                                   cv::Mat&                   descriptors)
{
...
boost::optional<Imath::M33d> cv_autoalign_node_t::find_similarity(
    const std::vector<cv::Point2d>& from_pts, const std::vector<cv::Point2d>& to_pts)
{
...
boost::optional<Imath::M33d> cv_autoalign_node_t::find_homography(
    const std::vector<cv::Point2d>& from_pts, const std::vector<cv::Point2d>& to_pts)
{
...
// rendering
void cv_autoalign_node_t::get_frame(const render::context_t& context,
                                    cv::Mat&                 dst,
                                    cv::Mat&                 msk,
                                    int                      use_alpha)
{
    ...
    if (!area.isEmpty())
    {
        ...
        if (use_alpha)
            image::alpha_subimage_to_cvmat_8u(
                inode->const_subimage_view(area), msk, offset.x, offset.y);
    }
    else
    {
        // clear with zeros.
