...
{
    track_callback(QProgressDialog* p, int value)
    : progress_(p)
    , value_(value)
    {
    }
...
private:
    ...
    {
        const ui::viewer::image_viewer_strategy_t* view
...
autostab_node_t::autostab_node_t()
: cv_autoalign_node_t()
{
    set_name("autostab");
...
autostab_node_t::autostab_node_t(const autostab_node_t& other)
: cv_autoalign_node_t(other)
{
    param_set().param_changed.connect(boost::bind(&autostab_node_t::param_changed, this, _1));
}
...
Imath::M33d autostab_node_t::do_calc_transform_matrix_at_frame(float frame, int subsample) const
{
    ...
    {
        return Imath::M33d(0, 0, 0, 0, 0, 0, 0, 0, 0);
    }
}
...
void autostab_node_t::track()
{
    ...
    QProgressDialog progress("Tracking",
                             "Cancel",
                             0,
                             end - start + 1,
...
void autostab_node_t::align_points(int                              frame,
                                   const std::vector<cv::KeyPoint>& ref_pts,
...
static bool registered
    = node_factory_t::Instance().register_node(autostab_node_t::autostab_node_metaclass());
