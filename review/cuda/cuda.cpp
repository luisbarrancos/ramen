...
bool delay_load_libcuda_internal()
{
    ...
    if (!(cuDriverGetVersion_
          = reinterpret_cast<CUresult (*)(int*)>(get_proc_ex("cuDriverGetVersion"))))
        return false;

    if (!(cuDeviceGet_
          = reinterpret_cast<CUresult (*)(CUdevice*, int)>(get_proc_ex("cuDeviceGet"))))
        return false;

    if (!(cuCtxCreate_ = reinterpret_cast<CUresult (*)(CUcontext*, unsigned int, CUdevice)>(
              get_proc_ex_v2("cuCtxCreate"))))
        return false;

    if (!(cuCtxDestroy_
          = reinterpret_cast<CUresult (*)(CUcontext)>(get_proc_ex_v2("cuCtxDestroy"))))
        return false;

    if (!(cuCtxPushCurrent_
          = reinterpret_cast<CUresult (*)(CUcontext)>(get_proc_ex_v2("cuCtxPushCurrent"))))
        return false;

    if (!(cuCtxPopCurrent_
          = reinterpret_cast<CUresult (*)(CUcontext*)>(get_proc_ex_v2("cuCtxPopCurrent"))))
        return false;

    if (!(cuCtxGetCurrent_
          = reinterpret_cast<CUresult (*)(CUcontext*)>(get_proc_ex("cuCtxGetCurrent"))))
        return false;
