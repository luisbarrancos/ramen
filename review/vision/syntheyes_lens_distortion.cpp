...
{
public:
    distort_fun(float k, float k3, float c)
    : k_(k)
    , k3_(k3)
    , c_(c)
    {
...
float redistort_scale(float k, float k3, float dist)
{
    ...
    float newdist = boost::math::tools::halley_iterate(
        distort_fun(k, k3, dist), best_guess, 0.0f, 5.0f, digits);
    return newdist / dist;
}

syntheyes_distortion_base::syntheyes_distortion_base(float               k,
                                                     float               k3,
...
syntheyes_quadratic_undistort::syntheyes_quadratic_undistort(float               k,
                                                             const Imath::Box2i& format,
                                                             float               pixel_aspect)
: syntheyes_distortion_base(k, 0, format, pixel_aspect)
{
}
...
syntheyes_undistort::syntheyes_undistort(float               k,
                                         float               k3,
                                         const Imath::Box2i& format,
                                         float               pixel_aspect)
: syntheyes_distortion_base(k, k3, format, pixel_aspect)
{
}
...
syntheyes_quadratic_redistort::syntheyes_quadratic_redistort(float               k,
                                                             const Imath::Box2i& format,
                                                             float               pixel_aspect)
: syntheyes_distortion_base(k, 0, format, pixel_aspect)
{
...
syntheyes_redistort::syntheyes_redistort(float               k,
                                         float               k3,
                                         const Imath::Box2i& format,
                                         float               pixel_aspect)
: syntheyes_distortion_base(k, k3, format, pixel_aspect)
