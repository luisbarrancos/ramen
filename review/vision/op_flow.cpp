...
void convert_to_bw(const CImg<>& src, CImg<>& dst)
{
    for (int j = 0; j < src.height(); ++j)
    {
        ...
        {
            dst(i, j)
                = ((0.3f * src(i, j, 0, 0)) + (0.59f * src(i, j, 0, 1)) + (0.11f * src(i, j, 0, 2)))
                  / 255.0f;
        }
    }
}
...
void animate_warp(const CImg<unsigned char>& src,
                  const CImg<unsigned char>& dest,
                  const CImg<>&              U,
                  int                        nb,
                  CImgDisplay&               disp)
{
...
float ssd(const CImg<>&     I1,
          const Imath::V2i& patch_pos,
          int               patch_w,
          int               patch_h,
          const CImg<>&     I2,
          const Imath::V2i& search_pos)
{
    float result = 0;

    for (int j = 0; j < patch_h; ++j)
        for (int i = 0; i < patch_w; ++i)
            result += square(I1(patch_pos.x + i, patch_pos.y + j)
                             - I2(search_pos.x + i, search_pos.y + j));

    return result;
}

void ssd_search(const CImg<>&       I1,
                const Imath::Box2i& patch_area,
                const CImg<>&       I2,
                const Imath::Box2i& search_area,
                Imath::V2i&         min_ssd_pos)
{
...
void do_block_match_optflow(const tbb::blocked_range<int>& r,
                            const CImg<>&                  I1,
                            const CImg<>&                  I2,
                            CImg<>&                        U,
                            int                            block_radius,
...
void block_match_optflow(
    const CImg<>& I1, const CImg<>& I2, CImg<>& U, int block_radius, int search_radius)
{
    tbb::parallel_for(tbb::blocked_range<int>(0, I1.height()),
                      boost::bind(do_block_match_optflow,
                                  _1,
                                  boost::cref(I1),
                                  boost::cref(I2),
                                  boost::ref(U),
...
// warp
void warping(const tbb::blocked_range<int>& r,
             const CImg<>&                  I1,
             const CImg<>&                  I2,
             const CImg<>&                  flow,
             CImg<>&                        I2_warped,
    ...
    for (int j = 0, je = p.height(); j < je; ++j)
    {
        for (int i = 0, ie = p.width(); i < ie; ++i)
        {
            ...
            float reproj = std::max(std::sqrt(square(p(i, j, 0, 0)) + square(p(i, j, 0, 1))
                                              + square(p(i, j, 0, 2)) + square(p(i, j, 0, 3))),
                                    1.0f);
...
void tvl1_motion_primal_dual(const CImg<>& I1,
                             const CImg<>& I2,
                             CImg<>&       flow,
                             CImg<>&       w,
                             CImg<>&       p,
                             ...
                             CImgDisplay&  disp)
{
    ...
    for (int warp = 0; warp < num_warps; ++warp)
    {
        ...
        if (disp)
        {
            ...
            tmp.resize(disp.width(), disp.height())
                .draw_quiver(flow0, white, 0.7f, 15, -14, true)
                .display(disp);
        }
        ...
        for (int iter = 0; iter < max_iters; ++iter)
        {
            ...
            for (int j = 0, je = Ix.height(); j < je; ++j)
            {
                for (int i = 0, ie = Ix.width(); i < ie; ++i)
                {
                    ...
                    float rho = It(i, j) + (flow(i, j, 0, 0) - flow0(i, j, 0, 0)) * Ix(i, j)
                                + (flow(i, j, 0, 1) - flow0(i, j, 0, 1)) * Iy(i, j)
                                + gamma * w(i, j);

                    float grad_sqr
                        = std::max(square(Ix(i, j)) + square(Iy(i, j)) + gamma * gamma, 1e-09f);
            ...
            if (disp && (iter % 10) == 0)
            {
                ...
                CImg<unsigned char> tmp     = I2_warped.get_normalize(0, 255);
                tmp.resize(disp.width(), disp.height())
                    .draw_quiver(flow, white, 0.7f, 15, -14, true)
                    .display(disp);
            }
        }
    }
...
void multiscale_optical_flow(const CImg<>& I1,
                             const CImg<>& I2,
                             float         scale_factor,
                             float         lambda,
                             int           num_warps,
                             ...
                             CImg<>&       w)
{
    ...
    else
    {
        ...
        for (int level = level_sizes.size() - 1; level >= 0; --level)
        {
            ...
            CImg<> pI1
                = I1.get_resize(level_sizes[level].first, level_sizes[level].second, 1, 1, 2);
            CImg<> pI2
                = I2.get_resize(level_sizes[level].first, level_sizes[level].second, 1, 1, 2);
            ...
            {
                std::cout << "Level = " << level << ", Size = " << level_sizes[level].first << ", "
                          << level_sizes[level].second << " Block Matching\n";
            ...
            {
                std::cout << "Level = " << level << ", Size = " << level_sizes[level].first << ", "
                          << level_sizes[level].second << " Variational\n";

                tvl1_motion_primal_dual(
                    pI1, pI2, flow, w, p, lambda, num_warps, max_iters, scale, disp);
            }
        }
    }
}
...
void smooth_flow(const CImg<>&     I1,
                 const CImg<>&     I2,
                 float             smoothness,
                 float             precision,
                 const CImgList<>& dI,
    ...
    for (unsigned int iteration = 0; iteration < max_iters; ++iteration)
    {
        ...
        cimg_for3XY(U, x, y)
        {
            ...
            cimg_forC(U, c)
            {
                ...
                U(x, y, c)
                    = (float) (U(x, y, c)
                               + dt * (deltaI * dI[c].linear_atXY(X, Y) + smoothness * (Uxx + Uyy)))
                      * denom;
...
int main(int argc, char** argv)
{
    ...
    const char* name_i1
        = cimg_option("-i", "/home/est/Devel/opflow/eval-data/Urban/frame10.png", "Input Image 1");
    const char* name_i2
        = cimg_option("-i2", "/home/est/Devel/opflow/eval-data/Urban/frame11.png", "Input Image 2");
    ...
    if (detail == 1.0f)
    {
        multiscale_optical_flow(
            I1, I2, 1.0f, lambda, num_warps, iters, blevels, bsize, bsearch, disp, flow, illum);
    ...
    else
    {
        ...
        multiscale_optical_flow(
            sI1, sI2, factor, lambda, num_warps, iters, blevels, bsize, bsearch, disp, flow, illum);
