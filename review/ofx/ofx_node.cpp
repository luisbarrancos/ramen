...
ofx_node_t::ofx_node_t(OFX::Host::ImageEffect::ImageEffectPlugin* plugin)
: image_node_t()
, descriptor_(0)
, effect_(0)
{
    ...
    param_changed_connection_
        = param_set().param_changed.connect(boost::bind(&ofx_node_t::param_changed, this, _1, _2));
}

ofx_node_t::ofx_node_t(const ofx_node_t& other)
: image_node_t(other)
, id_(other.id_)
, effect_(0)
{
...
void ofx_node_t::create_effect_instance()
{
    ...
    RAMEN_ASSERT(!effect_
                 && "OFX: trying to create an effect instance for a node which already has one");
...
void ofx_node_t::param_changed(param_t* p, param_t::change_reason reason)
{
    ...
    {
#ifndef NDEBUG
        DLOG(INFO) << "ofx_node_t::param_changed, node = " << name() << ", param = " << p->id()
        ...
        switch (reason)
        {
            case param_t::user_edited:
                ...
                effect_->paramInstanceChangedAction(
                    p->id(), kOfxChangeUserEdited, frame, renderscale);
            ...
            case param_t::node_edited:
                ...
                effect_->paramInstanceChangedAction(
                    p->id(), kOfxChangePluginEdited, frame, renderscale);
...
void ofx_node_t::do_connected(node_t* src, int port)
{
    if (effect_)
    {
        ...
        for (int i = 0; i < effect_->getNClips(); ++i)
        {
            ...
            if (clip->port() == port)
                effect_->clipInstanceChangedAction(
                    clip->getName(), kOfxChangeUserEdited, time, renderscale);
        }
...
void ofx_node_t::do_calc_inputs_interest(const render::context_t& context)
{
    ...
    for (std::map<OFX::Host::ImageEffect::ClipInstance*, OfxRectD>::const_iterator it(rois.begin());
         it != rois.end();
         ++it)
    {
        ofx::clip_t* clip = dynamic_cast<ofx::clip_t*>(it->first);
        Imath::Box2i box(Imath::V2i(it->second.x1, it->second.y1),
                         Imath::V2i(it->second.x2 - 1, it->second.y2 - 1));
...
#ifndef NDEBUG
    DLOG(INFO) << "node " << name() << ", beginRenderAction, frame = " << context.frame
               << " scale = " << scale;
#endif

    effect_->beginRenderAction(
        context.frame, context.frame, 1, context.mode == render::interface_render, scale);
...
#ifndef NDEBUG
    DLOG(INFO) << "node " << name() << ", endRenderAction, frame = " << context.frame
               << " scale = " << scale << "\n\n";
#endif

    effect_->endRenderAction(
        context.frame, context.frame, 1, context.mode == render::interface_render, scale);
}
