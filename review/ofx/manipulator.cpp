...
manipulator_t::manipulator_t(image::ofx_node_t* node, image_effect_t* effect)
: ramen::manipulator_t()
, OFX::Host::Interact::Instance(effect->getOverlayDescriptor(8, true), (void*) effect)
{
...
bool manipulator_t::do_mouse_press_event(const ui::mouse_press_event_t& event)
{
    ...
    penDownAction(node->composition()->frame(),
                  get_render_scale(node, event),
                  get_pen_pos(node, event),
                  get_mouse_pos(node, event),
                  1);
    return true;
}
...
void manipulator_t::do_mouse_move_event(const ui::mouse_move_event_t& event)
{
    ...
    penMotionAction(node->composition()->frame(),
                    get_render_scale(node, event),
                    get_pen_pos(node, event),
                    get_mouse_pos(node, event),
                    0);
...
void manipulator_t::do_mouse_release_event(const ui::mouse_release_event_t& event)
{
    ...
    penUpAction(node->composition()->frame(),
                get_render_scale(node, event),
                get_pen_pos(node, event),
                get_mouse_pos(node, event),
                0);
...
OfxPointD manipulator_t::get_render_scale(const image::ofx_node_t* node,
                                          const ui::mouse_event_t& event) const
{
    OfxPointD render_scale;
...
OfxPointI manipulator_t::get_mouse_pos(const image::ofx_node_t* node,
                                       const ui::mouse_event_t& event) const
{
    OfxPointI mouse_pos;
...
OfxPointD manipulator_t::get_pen_pos(const image::ofx_node_t* node,
                                     const ui::mouse_event_t& event) const
{
    RAMEN_ASSERT(node);
...
{
    ui::viewer::image_viewer_strategy_t* iview = dynamic_cast<ui::viewer::image_viewer_strategy_t*>(
        &ui::viewer_t::Instance().current_viewer());
    RAMEN_ASSERT(iview);
