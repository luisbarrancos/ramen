...
namespace ofx
{
memory_instance_t::memory_instance_t()
...
bool memory_instance_t::alloc(std::size_t nBytes)
{
    if (!_locked)
    {
        ...
        _size = nBytes;
        memory::manager_t::image_allocator_type& alloc(
            application_t::Instance().memory_manager().image_allocator());
        _ptr = reinterpret_cast<char*>(alloc.allocate(_size));

#ifndef NDEBUG
        DLOG(INFO) << "memory_instance_t::alloc, this = " << (void*) this << " size = " << _size
                   << " ptr = " << (void*) _ptr;
#endif
...
#ifndef NDEBUG
    DLOG(INFO) << "memory_instance_t::alloc, this = " << (void*) this << " size = " << nBytes
               << ". failed, memory was locked";
#endif

    return false;
}

OfxImageMemoryHandle memory_instance_t::getHandle()
{
    return OFX::Host::Memory::Instance::getHandle();
}
...
void memory_instance_t::freeMem()
{
    ...
    {
#ifndef NDEBUG
        DLOG(INFO) << "memory_instance_t::freeMem, this = " << (void*) this << " size = " << _size
                   << " ptr = " << (void*) _ptr;
#endif
