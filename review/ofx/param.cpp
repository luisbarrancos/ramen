                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (d->getType() == kOfxParamTypeString)
                                            p.reset(create_string_param(d));
                                        else
                                        {
#ifndef NDEBUG
                                            DLOG(INFO) << "OFX Host: param not implemented:"
                                                       << d->getType();
#endif
...
param_t* param_factory_t::create_choice_param(OFX::Host::Param::Descriptor* d)
{
    ...
    for (int i = 0; i < num_items; ++i)
        p->menu_items().push_back(d->getProperties().getStringProperty(key, i));
...
param_t* param_factory_t::create_string_param(OFX::Host::Param::Descriptor* d)
{
    ramen::string_param_t* p = new ramen::string_param_t(d->getShortLabel());
    p->set_default_value(d->getProperties().getStringProperty(kOfxParamPropDefault, 0));
    return p;
}
