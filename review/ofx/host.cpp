...
: OFX::Host::ImageEffect::Host()
{
    ...
    _properties.setIntProperty(kOfxImageEffectHostPropIsBackground,
                               application_t::Instance().command_line());
    ...
    _properties.setStringProperty(
        kOfxImageEffectPropSupportedContexts, kOfxImageEffectContextFilter, 1);
    _properties.setStringProperty(
        kOfxImageEffectPropSupportedContexts, kOfxImageEffectContextGeneral, 2);
    _properties.setStringProperty(
        kOfxImageEffectPropSupportedContexts, kOfxImageEffectContextTransition, 3);
...
bool host_impl::pluginSupported(OFX::Host::ImageEffect::ImageEffectPlugin* plugin,
                                std::string&                               reason) const
{
    ...
    {
#ifndef NDEBUG
        DLOG(INFO) << "OFX Host: plugin " << plugin->getIdentifier()
                   << " not supported: No supported context";
#endif
...
#ifndef NDEBUG
        DLOG(INFO) << "OFX Host: plugin " << plugin->getIdentifier()
                   << " not supported: Float bit depth not supported by plugin";
#endif
    ...
    {
        DLOG( INFO) << "OFX Host: plugin " << plugin->getIdentifier() << " not supported: Plugin is
    single instance"; reason = "Single instance plugins not supported"; return false;
    }
    */
...
OFX::Host::ImageEffect::Instance* host_impl::newInstance(
    void*                                      clientData,
    OFX::Host::ImageEffect::ImageEffectPlugin* plugin,
    OFX::Host::ImageEffect::Descriptor&        desc,
    const std::string&                         context)
{
    return new image_effect_t(
        plugin, desc, context, reinterpret_cast<image::ofx_node_t*>(clientData));
}

OFX::Host::ImageEffect::Descriptor* host_impl::makeDescriptor(
    OFX::Host::ImageEffect::ImageEffectPlugin* plugin)
{
...
#ifndef NDEBUG
    if (result == 0)
        DLOG(INFO) << "OFX Host: suite does not exist: " << suiteName
