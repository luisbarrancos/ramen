...
: OFX::Host::ImageEffect::ClipInstance(effectInstance, desc)
{
...
const std::string& clip_t::getUnmappedComponents() const
{
    ...
    {
#ifndef NDEBUG
        DLOG(INFO) << "clip_t::getUnmappedComponents, node = " << node()->name()
                   << ", port = " << port() << ", result = srgba";
#endif
...
#ifndef NDEBUG
    DLOG(INFO) << "clip_t::getAspectRatio, node = " << node()->name() << ", port = " << port()
               << ", result = " << aspect;
#endif
...
#ifndef NDEBUG
    DLOG(INFO) << "clip_t::getRoD, node = " << node()->name() << ", port = " << port()
               << ", result = " << v;
#endif
...
image_t* clip_t::get_input_image(OfxTime time, OfxRectD* optionalBounds)
{
    ...
    if (optionalBounds)
    {
        ...
        area = Imath::Box2i(Imath::V2i(optionalBounds->x1, optionalBounds->y1),
                            Imath::V2i(optionalBounds->x2 - 1, optionalBounds->y2 - 1));
    ...
    if (getComponents() == kOfxImageComponentRGBA)
    {
        ...
        result    = new image_t(*this,
                             pixels,
                             1.0 / node()->render_context().subsample,
                             ptr,
                             bounds,
                             bounds,
                             rowbytes,
