...
: OFX::Host::Param::StringInstance(descriptor, instance)
{
...
OfxStatus string_param_t::copy(const OFX::Host::Param::Instance& instance,
                               OfxTime                           offset,
                               OfxRangeD                         range)
{
    return missing_host_feature();
...
{
    ramen::string_param_t& param(
        dynamic_cast<ramen::string_param_t&>(effect_->node()->param(getName())));
