...
{
    const std::vector<boost::filesystem::path> more_paths(
        ramen::plugin_manager_t::Instance().paths());

    OFX::Host::PluginCache::getPluginCache()->setCacheVersion("RamenV1");

    for (int i = 0; i < more_paths.size(); ++i)
        OFX::Host::PluginCache::getPluginCache()->addFileToPath(
            filesystem::file_string(more_paths[i]));
    ...
    std::map<std::string, OFX::Host::ImageEffect::ImageEffectPlugin*>::const_iterator it(
        image_effect_plugin_cache_->getPluginsByID().begin());
    std::map<std::string, OFX::Host::ImageEffect::ImageEffectPlugin*>::const_iterator end(
        image_effect_plugin_cache_->getPluginsByID().end());
...
node_ptr_t plugin_manager_impl::create_node_by_id(std::string id,
                                                  int         major_version,
                                                  int         minor_version)
{
    OFX::Host::ImageEffect::ImageEffectPlugin* plugin
        = image_effect_plugin_cache_->getPluginById(id, major_version, minor_version);
...
{
    OFX::Host::ImageEffect::ImageEffectPlugin* plugin
        = image_effect_plugin_cache_->getPluginByLabel(label);
    return create_node(plugin);
}
...
OFX::Host::ImageEffect::ImageEffectPlugin* plugin_manager_impl::get_plugin_by_id(
    const std::string& id, int major, int minor)
{
