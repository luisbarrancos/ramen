...
: OFX::Host::Param::Double2DInstance(descriptor, instance)
{
...
OfxStatus double2d_param_t::copy(const OFX::Host::Param::Instance& instance,
                                 OfxTime                           offset,
                                 OfxRangeD                         range)
{
    const double2d_param_t* other = dynamic_cast<const double2d_param_t*>(&instance);
...
{
    const float2_param_t& param(
        dynamic_cast<const float2_param_t&>(effect_->node()->param(getName())));
