...
OfxStatus integer_param_t::copy(const OFX::Host::Param::Instance& instance,
                                OfxTime                           offset,
                                OfxRangeD                         range)
{
    const integer_param_t* other = dynamic_cast<const integer_param_t*>(&instance);
    effect_->node()->param(getName()).copy(
        other->effect_->node()->param(other->getName()), offset, range.min, range.max);
    return kOfxStatOK;
}
...
OfxStatus integer_param_t::derive(OfxTime time, int& v) { return kOfxStatErrMissingHostFeature; }
...
{
    const float_param_t& param(
        dynamic_cast<const float_param_t&>(effect_->node()->param(getName())));
