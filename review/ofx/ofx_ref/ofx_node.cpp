...
, descriptor_(0)
{
    ...
    param_changed_connection_
        = param_set().param_changed.connect(boost::bind(&ofx_node_t::param_changed, this, _1));
}
...
const ofx::ofx_manipulator_t* ofx_node_t::overlay() const
{
    ...
    const ofx::ofx_manipulator_t* manip
        = dynamic_cast<const ofx::ofx_manipulator_t*>(&(manipulators()[0]));
    return manip;
}
...
void ofx_node_t::do_process(const render::render_context_t& context)
{
    ...
    effect_->beginRenderAction(
        context.time, context.time, 1, context.mode == render::interface_render, scale);
...
void ofx_node_t::serialize(boost::archive::polymorphic_iarchive& ar, const unsigned int version)
{
    ...
    plugin_ = application_t::Instance().ofx_plugin_manager().get_plugin_by_id(
        id_, major_version_, minor_version_);
    ar& boost::serialization::make_nvp("ofx_context", ofx_context_);
}

void ofx_node_t::serialize(boost::archive::polymorphic_oarchive& ar,
