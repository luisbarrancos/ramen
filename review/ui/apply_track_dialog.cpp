...
apply_track_dialog_t::apply_track_dialog_t()
...
const image::tracker_node_t* apply_track_dialog_t::exec(image::apply_track_mode& mode,
                                                        image::apply_track_use&  use)
{
    std::vector<const image::tracker_node_t*> trackers;
    ui_.tracker_combo_->clear();

    for (const node_t& n : app().document().composition().nodes())
    {
    ...
    {
        QMessageBox::warning(
            app().ui()->main_window(), "Ramen", "No active tracker nodes in composition");
        return 0;
    }
