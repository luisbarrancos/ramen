...
namespace imageio
{
flo_reader_t::flo_reader_t(const boost::filesystem::path& p)
: reader_t(p)
{
...
void flo_reader_t::do_read_image(const image::image_view_t& view,
                                 ...
                                 int                        subsample) const
{
    ...
    if (fread(&tag, sizeof(float), 1, fp.get()) != 1 || fread(&width, sizeof(int), 1, fp.get()) != 1
