...
{
    se_expr_warp_fun(distortx_node_t*         node,
                     const std::string        str,
                     const Imath::V2f         max_amplitude,
                     const render::context_t& context)
    : expr_(str, node, base_expr_node_t::warp_context)
    {
        RAMEN_ASSERT(node);
        node_          = node;
    ...
    se_expr_warp_fun(const se_expr_warp_fun& other)
    : expr_(other.string(), other.node_, base_expr_node_t::warp_context)
    {
        node_          = other.node_;
        format_        = node_->format();
...
distortx_node_t::distortx_node_t()
: base_expr_node_t()
{
    set_name("distortx");
...
distortx_node_t::distortx_node_t(const distortx_node_t& other)
...
void distortx_node_t::do_create_params()
{
    std::auto_ptr<se_expr_param_t> s(
        new se_expr_param_t("Warp Expr", boost::bind(&base_expr_node_t::make_expression, this)));
...
static bool registered
    = node_factory_t::instance().register_node(distortx_node_t::distortx_node_metaclass());
