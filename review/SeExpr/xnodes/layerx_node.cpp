...
layerx_node_t::layerx_node_t(const layerx_node_t& other)
...
void layerx_node_t::do_create_params()
{
    ...
    std::auto_ptr<se_expr_param_t> s(
        new se_expr_param_t("Layer Expr", boost::bind(&base_expr_node_t::make_expression, this)));
...
void layerx_node_t::do_expression(const tbb::blocked_range<int>& range,
                                  const render::context_t&       context)
{
    ...
    for (p.y = range.begin(); p.y < range.end(); ++p.y)
    {
        ...
        for (p.x = defined().min.x; p.x <= xe; ++p.x)
        {
            ...
            {
                image::pixel_t q = bg_view(p.x - bg->defined().min.x, p.y - bg->defined().min.y);
            ...
            {
                image::pixel_t q = fg_view(p.x - fg->defined().min.x, p.y - fg->defined().min.y);
...
static bool registered
    = node_factory_t::instance().register_node(layerx_node_t::layerx_node_metaclass());
