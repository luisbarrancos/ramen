...
namespace image
{
base_expr_node_t::base_expr_node_t()
: image_node_t()
{
    add_output_plug();
...
base_expr_node_t::base_expr_node_t(const base_expr_node_t& other)
: image_node_t(other)
{
    param_set().param_changed.connect(boost::bind(&base_expr_node_t::param_changed, this, _1, _2));
}
...
base_expr_node_t::image_expression_t::image_expression_t(const std::string&      expr,
                                                         const base_expr_node_t* n,
                                                         expression_context      context)
: se_expression_t(expr)
{
    context_ = context;
...
void base_expr_node_t::image_expression_t::setup_variables(const base_expr_node_t*  n,
                                                           const render::context_t& rcontext)
{
...
double* base_expr_node_t::image_expression_t::get_local_var_ref(const std::string& name,
                                                                double*            default_ref)
{
...
SeVec3d* base_expr_node_t::image_expression_t::get_local_var_ref(const std::string& name,
                                                                 SeVec3d*           default_ref)
{
    const SeExpression::LocalVarTable&          vtable = getLocalVars();
    SeExpression::LocalVarTable::const_iterator it     = vtable.find(name);
