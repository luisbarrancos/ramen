...
namespace image
{
generatex_node_t::generatex_node_t()
: base_expr_node_t()
{
    set_name("generateX");
}

generatex_node_t::generatex_node_t(const generatex_node_t& other)
: base_expr_node_t(other)
{
    create_expression();
}
...
void generatex_node_t::do_create_params()
{
    ...
    std::auto_ptr<se_expr_param_t> s(
        new se_expr_param_t("Color Expr", boost::bind(&base_expr_node_t::make_expression, this)));
...
void generatex_node_t::do_calc_bounds(const render::context_t& context) { set_bounds(format()); }
...
void generatex_node_t::do_process(const render::context_t& context)
{
    ...
    if (expr_->isThreadSafe())  // multi-threaded
    {
        tbb::parallel_for(
            ...
            boost::bind(
                &generatex_node_t::do_aa_expression, this, _1, boost::cref(context), samples),
...
void generatex_node_t::do_expression(const tbb::blocked_range<int>& range,
...
void generatex_node_t::do_aa_expression(const tbb::blocked_range<int>& range,
                                        const render::context_t&       context,
                                        int                            samples)
{
    if (samples == 1)
...
static bool registered
    = node_factory_t::instance().register_node(generatex_node_t::generatex_node_metaclass());
