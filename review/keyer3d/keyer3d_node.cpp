...
struct key3d_fun
{
    key3d_fun(const keyer3d::km_keyer_t& keyer)
    : keyer_(keyer)
    {
    }
...
keyer3d_node_t::keyer3d_node_t(const keyer3d_node_t& other)
: keyer_node_t(other)
, keyer_(other.keyer_)
, toolbar_(0)
{
...
void keyer3d_node_t::do_process(const render::context_t& context)
{
    ...
    if (!input(1))
    {
        ...
        boost::gil::tbb_transform_pixels(
            in0->const_subimage_view(area), subimage_view(area), key3d_fun(keyer()));
    }
    else
    {
...
void keyer3d_node_t::do_read(const serialization::yaml_node_t& node,
...
static bool registered
    = node_factory_t::instance().register_node(keyer3d_node_t::keyer3d_node_metaclass());
