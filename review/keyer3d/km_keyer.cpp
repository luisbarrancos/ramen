...
namespace keyer3d
{
km_keyer_t::cluster_t::cluster_t()
: center(0, 0, 0)
{
    r_tol = r_soft = 0;
}

km_keyer_t::cluster_t::cluster_t(const Imath::Color3f& c)
: center(c)
{
    r_tol = r_soft = 0;
}

float km_keyer_t::cluster_t::operator()(const Imath::Color3f& c,
                                        float                 tol_factor,
                                        float                 soft_factor) const
{
...
bool km_keyer_t::cluster_t::inside_softness_region(const Imath::Color3f& color,
                                                   float                 tol_factor,
                                                   float                 soft_factor) const
{
...
km_keyer_t::km_keyer_t(int max_clusters)
...
void km_keyer_t::add_to_hash_str(hash::generator_t& gen) const
{
    for (int i = 0; i < clusters().size(); ++i)
    {
        gen << clusters()[i].center.x << clusters()[i].center.y << clusters()[i].center.z
            << clusters()[i].r_tol << clusters()[i].r_soft;
    }
}
...
void km_keyer_t::write(serialization::yaml_oarchive_t& out) const
{
    ...
    for (int i = 0; i < clusters().size(); ++i)
    {
        out << clusters()[i].center.x << clusters()[i].center.y << clusters()[i].center.z
            << clusters()[i].r_tol << clusters()[i].r_soft;
    }
    out.end_seq();
}
