...
Imath::Box2i scale(const Imath::Box2i& b, float sx, float sy)
{
    return Imath::Box2i(
        Imath::V2i(
            Imath::Math<float>::floor(b.min.x * sx),
            Imath::Math<float>::floor(b.min.y * sy)),
        Imath::V2i(
            Imath::Math<float>::ceil(b.max.x * sx),
            Imath::Math<float>::ceil(b.max.y * sy)));
}
...
{
    return Imath::Box2f(
        Imath::V2f(b.min.x * sx, b.min.y * sy),
        Imath::V2f(b.max.x * sx, b.max.y * sy));
}
