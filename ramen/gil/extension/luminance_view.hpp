...
namespace detail
{
template <typename SrcP> class luminance_deref_fn
{
  ...
  public:
    ...
    typedef
        typename pixel_value_type<channel_t, gray_layout_t>::type value_type;
    typedef
        typename pixel_reference_type<channel_t, gray_layout_t, false, false>::
    ...
    template <typename P> luminance_deref_fn(const luminance_deref_fn<P>& d) {}
...
template <typename View> struct __luminance_view<View>
{
  private:
...
template <typename View> struct luminance_view_type
{
  private:
