...
namespace detail
{
template<class View, bool IsAdjacent>
struct __rgb_from_rgba_view_basic;

template<class View>
struct __rgb_from_rgba_view_basic<View, false>
{
    typedef typename view_type<typename channel_type<View>::type,
                               rgb_layout_t,
                               true,
                               true,
...
struct __rgb_from_rgba_view_basic<View, true>
{
    ...
    static type make(const View& src)
    {
        ...
        return planar_rgb_view(src.width(),
                               src.height(),
                               (x_iterator_t) & (get_color(src(0, 0), red_t())),
...
private:
    ...
    // Planar and grayscale iterators have channels adjacent in memory, whereas multi-channel
    // interleaved and iterators with non-fundamental step do not.
...
public:
    ...
    typedef
        typename pixel_reference_type<channel_t, rgb_layout_t, false, false>::type const_reference;
    ...
    {
        return result_type(
            get_color(srcP, red_t()), get_color(srcP, green_t()), get_color(srcP, blue_t()));
    }
};

template<typename View>
struct __rgb_from_rgba_view<View, false>
{
private:
