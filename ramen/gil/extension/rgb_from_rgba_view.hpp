...
template <class View> struct __rgb_from_rgba_view_basic<View, false>
{
    ...
    static type make(const View& src)
    {
        ...
        x_iterator_base_t base_it(
            &(get_color(src(0, 0), red_t())),
...
template <class View> struct __rgb_from_rgba_view_basic<View, true>
{
    typedef typename view_type<
        typename channel_type<View>::type,
        rgb_layout_t,
...
template <typename SrcP> class rgb_from_rgba_deref_fn
{
    BOOST_STATIC_CONSTANT(
        bool,
    ...
    typedef typename pixel_reference_type<
        channel_t,
        rgb_layout_t,
        false,
  ...
  public:
    ...
    typedef
        typename pixel_reference_type<channel_t, rgb_layout_t, false, false>::
    ...
    {
        return result_type(
            get_color(srcP, red_t()),
            get_color(srcP, green_t()),
            get_color(srcP, blue_t()));
    }
};

template <typename View> struct __rgb_from_rgba_view<View, false>
{
  private:
...
template <typename View> struct rgb_from_rgba_view_type
{
  private:
