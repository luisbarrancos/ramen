...
struct checks_fn
{
    ...
    checks_fn(const point_t& sz, std::size_t check_size, const Pixel& color0, const Pixel& color1)
    : size_(sz)
...
void premult_alpha(const RGBAView& dst)
{
    for_each_pixel(dst, detail::alpha_premult_t());
}

template<class RGBAView>
void unpremult_alpha(const RGBAView& dst)
{
...
template<class Pixel>
image_view<virtual_2d_locator<detail::checks_fn<Pixel>, false>> make_checks_view(
