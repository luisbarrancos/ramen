...
namespace detail
{
template <class Channel> Channel chan_divide(Channel c, Channel a)
{
    float tmp = (float) c / a;
    return Channel(tmp);
}
...
struct alpha_premult_t
{
    ...
    typename PixelRef::value_type operator()(const PixelRef& p)
    {
        ...
        get_color(r, red_t()) =
            channel_multiply(get_color(p, red_t()), get_color(p, alpha_t()));
        get_color(r, green_t()) =
            channel_multiply(get_color(p, green_t()), get_color(p, alpha_t()));
        get_color(r, blue_t()) =
            channel_multiply(get_color(p, blue_t()), get_color(p, alpha_t()));
...
template <class Pixel> struct checks_fn
{
...
template <class RGBAView> void premult_alpha(const RGBAView& dst)
{
    for_each_pixel(dst, detail::alpha_premult_t());
}

template <class RGBAView> void unpremult_alpha(const RGBAView& dst)
{
    for_each_pixel(dst, detail::alpha_unpremult_t());
}
...
template <class RGBAView> void invert_alpha(const RGBAView& dst)
{
    for (int y = 0; y < dst.dimensions().y; ++y)
    {
        typename RGBAView::x_iterator dst_it = dst.row_begin(y);
        ...
        for (int x = 0; x < dst.dimensions().x; ++x)
            get_color(dst_it[x], alpha_t()) =
                channel_invert(get_color(dst_it[x], alpha_t()));
    }
}
...
make_checks_view(
    const point2<ptrdiff_t>& dims,
    std::size_t              checks_size,
    const Pixel&             color0,
    const Pixel&             color1)
{
