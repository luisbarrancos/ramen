...
// dst += DST_TYPE(src * w)
template <typename SrcP, typename Weight, typename DstP> struct add_dst_mul_src
{
...
// brief A sampler that sets the destination pixel as the bilinear interpolation
// of the four closest pixels from the source. If outside the bounds, it doesn't
// change the destination ingroup ImageAlgorithms
    ...
    DstP&            result)
{
    ...
    pixel<F, devicen_layout_t<num_channels<SrcView>::value>> mp(
        0);  // suboptimal
    ...
    if (p0.x + 1 < src.width())
    {
        ...
        {
            // most common case - inside the image, not on the last row or
            // column
