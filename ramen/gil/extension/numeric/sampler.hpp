...
///////////////////////////////////////////////////////////////////////////
////
////     resample_pixels: set each pixel in the destination view as the result of a sampling
/// function over the transformed coordinates of the source view
////
///////////////////////////////////////////////////////////////////////////
/*
...
concept SamplerConcept {
    template <typename DstP,      // Models PixelConcept
              ...
              typename S_COORDS>  // Models PointNDConcept, where S_COORDS::num_dimensions ==
SrcView::num_dimensions bool sample(const Sampler& s, const SrcView& src, const S_COORDS& p, DstP
result);
};
*/

// brief A sampler that sets the destination pixel to the closest one in the source. If outside the
...
{
    Weight _w;
    add_dst_mul_src_channel(Weight w)
    : _w(w)
    {
    }
...
// brief A sampler that sets the destination pixel as the bilinear interpolation of the four closest
// pixels from the source. If outside the bounds, it doesn't change the destination ingroup
// ImageAlgorithms
...
bool sample(bilinear_sampler, const SrcView& src, const point2<F>& p, DstP& result)
{
    ...
    if (p0.x + 1 < src.width())
    {
        if (p0.y + 1 < src.height())
        {
            ...
            detail::add_dst_mul_src<SrcP,
                                    F,
                                    pixel<F, devicen_layout_t<num_channels<SrcView>::value>>>()(
