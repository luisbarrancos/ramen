    ...
    Correlator               correlator)
{
        ...
        option == convolve_option_output_zero)
    {
        ...
        else
        {
            ...
            {
                assign_pixels(
                    src.row_begin(rr), src.row_end(rr), &buffer.front());
    ...
    else
    {
        ...
        for (int rr = 0; rr < height; ++rr)
        {
            ...
            {
                assign_pixels(
                    src.row_begin(rr) - ker.left_size(),
            ...
            {
                PixelAccum filler;
                pixel_assigns_t<PIXEL_SRC_REF, PixelAccum>()(
                    *src.row_begin(rr), filler);
                std::fill_n(it_buffer, ker.left_size(), filler);
                it_buffer += ker.left_size();
                assign_pixels(src.row_begin(rr), src.row_end(rr), it_buffer);
                it_buffer += width;
                pixel_assigns_t<PIXEL_SRC_REF, PixelAccum>()(
                    src.row_end(rr)[-1], filler);
...
template <typename PixelAccum> class correlator_n
{
  ...
  public:
    ...
    {
        correlate_pixels_n<PixelAccum>(
            src_begin, src_end, ker_begin, _size, dst_begin);
    }
};
template <std::size_t Size, typename PixelAccum> struct correlator_k
{
  public:
    ...
    void operator()(
        ...
        DstIterator    dst_begin)
    {
        correlate_pixels_k<Size, PixelAccum>(
            src_begin, src_end, ker_begin, dst_begin);
    }
};
}  // namespace detail
...
template <
    ...
    typename Kernel,
    typename DstView>
GIL_FORCEINLINE void correlate_rows(
    const SrcView&           src,
    const Kernel&            ker,
    const DstView&           dst,
...
template <
    ...
    typename Kernel,
    typename DstView>
GIL_FORCEINLINE void correlate_cols(
    const SrcView&           src,
    const Kernel&            ker,
    const DstView&           dst,
    convolve_boundary_option option = convolve_option_extend_zero)
{
    correlate_rows<PixelAccum>(
        transposed_view(src), ker, transposed_view(dst), option);
}
...
template <
    ...
    typename Kernel,
    typename DstView>
GIL_FORCEINLINE void convolve_rows(
    const SrcView&           src,
    const Kernel&            ker,
    const DstView&           dst,
    convolve_boundary_option option = convolve_option_extend_zero)
...
template <
    ...
    typename Kernel,
    typename DstView>
GIL_FORCEINLINE void convolve_cols(
    const SrcView&           src,
    const Kernel&            ker,
    const DstView&           dst,
    convolve_boundary_option option = convolve_option_extend_zero)
