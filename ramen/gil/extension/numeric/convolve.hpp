...
template<typename PixelAccum,
         ...
         typename DstView,
         typename Correlator>
void correlate_rows_imp(const SrcView&           src,
                        const Kernel&            ker,
                        const DstView&           dst,
                        ...
                        Correlator               correlator)
{
    ...
    if (option == convolve_option_output_ignore || option == convolve_option_output_zero)
    {
        ...
        else
        {
            ...
            for (int rr = 0; rr < height; ++rr)
            {
                ...
                correlator(
                    &buffer.front(), &buffer.front() + width + 1 - ker.size(), ker.begin(), it_dst);
    ...
    else
    {
        ...
        for (int rr = 0; rr < height; ++rr)
        {
            ...
            correlator(&buffer.front(), &buffer.front() + width, ker.begin(), dst.row_begin(rr));
        }
    }
}
template<typename PixelAccum>
...
GIL_FORCEINLINE void correlate_rows(const SrcView&           src,
                                    const Kernel&            ker,
                                    const DstView&           dst,
...
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void correlate_cols(const SrcView&           src,
                                    const Kernel&            ker,
                                    const DstView&           dst,
                                    convolve_boundary_option option = convolve_option_extend_zero)
{
...
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void convolve_rows(const SrcView&           src,
                                   const Kernel&            ker,
                                   const DstView&           dst,
                                   convolve_boundary_option option = convolve_option_extend_zero)
{
...
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void convolve_cols(const SrcView&           src,
                                   const Kernel&            ker,
                                   const DstView&           dst,
                                   convolve_boundary_option option = convolve_option_extend_zero)
{
...
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void correlate_rows_fixed(const SrcView&           src,
                                          const Kernel&            ker,
                                          const DstView&           dst,
                                          convolve_boundary_option option
...
{
    detail::correlate_rows_imp<PixelAccum>(
        src, ker, dst, option, detail::correlator_k<Kernel::static_size, PixelAccum>());
}

// \ingroup ImageAlgorithms
// correlate a 1D fixed-size kernel along the columns of an image
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void correlate_cols_fixed(const SrcView&           src,
                                          const Kernel&            ker,
                                          const DstView&           dst,
                                          convolve_boundary_option option
                                          = convolve_option_extend_zero)
...
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void convolve_rows_fixed(const SrcView&           src,
                                         const Kernel&            ker,
                                         const DstView&           dst,
                                         convolve_boundary_option option
                                         = convolve_option_extend_zero)
...
template<typename PixelAccum, typename SrcView, typename Kernel, typename DstView>
GIL_FORCEINLINE void convolve_cols_fixed(const SrcView&           src,
                                         const Kernel&            ker,
                                         const DstView&           dst,
                                         convolve_boundary_option option
                                         = convolve_option_extend_zero)
