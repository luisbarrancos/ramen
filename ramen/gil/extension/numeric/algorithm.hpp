...
namespace gil
{
// \brief Returns the reference proxy associated with a type that has a \p "reference" member
// typedef.
//
// The reference proxy is the reference type, but with stripped-out C++ reference. It models
// PixelConcept
template<typename T>
...
{
    typedef typename pixel_proxy<typename std::iterator_traits<SrcIterator>::value_type>::type
        PIXEL_SRC_REF;
    typedef typename pixel_proxy<typename std::iterator_traits<DstIterator>::value_type>::type
                                                                      PIXEL_DST_REF;
...
template<std::size_t Size,
         ...
         typename KernelIterator,
         typename DstIterator>
inline DstIterator correlate_pixels_k(SrcIterator    src_begin,
                                      SrcIterator    src_end,
                                      KernelIterator ker_begin,
                                      DstIterator    dst_begin)
{
