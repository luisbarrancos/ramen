...
namespace detail
{
template <class View1, class View2, class Fun> struct tbb_transform_pixels_fun
{
    tbb_transform_pixels_fun(const View1& src, const View2& dst, Fun f)
      : src_(src)
      , dst_(dst)
      , f_(f)
    {
    ...
    void operator()(const tbb::blocked_range<size_t>& r) const
    {
        for (std::size_t i = r.begin(), e = r.end(); i != e; ++i)
            std::transform(
                src_.row_begin(i), src_.row_end(i), dst_.row_begin(i), f_);
    }
...
tbb_transform_pixels_fun<View1, View2, Fun> make_tbb_transform_pixels_fun(
    const View1& src,
    const View2& dst,
    Fun          f)
{
...
template <class View1, class View2, class Fun>
GIL_FORCEINLINE void tbb_transform_pixels(
    const View1& src,
    const View2& dst,
    Fun          f)
{
