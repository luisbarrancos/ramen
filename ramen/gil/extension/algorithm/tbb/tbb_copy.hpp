...
namespace detail
{
template <class View1, class View2> struct tbb_copy_pixels_fun
{
    tbb_copy_pixels_fun(const View1& src, const View2& dst)
      : src_(src)
      , dst_(dst)
    {
    }
...
tbb_copy_pixels_fun<View1, View2> make_tbb_copy_pixels_fun(
    const View1& src,
    const View2& dst)
{
...
class tbb_copy_and_convert_pixels_fn
  : public binary_operation_obj<tbb_copy_and_convert_pixels_fn<CC>>
{
  private:
    CC _cc;
  ...
  public:
    typedef typename binary_operation_obj<
        tbb_copy_and_convert_pixels_fn<CC>>::result_type result_type;
    tbb_copy_and_convert_pixels_fn() {}

    tbb_copy_and_convert_pixels_fn(CC cc_in)
      : _cc(cc_in)
    {
    }
    // when the two color spaces are incompatible, a color conversion is
    // performed
    template <typename V1, typename V2>
    result_type apply_incompatible(const V1& src, const V2& dst) const
    {
        // copy_pixels( color_converted_view<typename V2::value_type>( src,
        // _cc),dst);
        tbb::parallel_for(
            tbb::blocked_range<std::size_t>(0, src.height()),
            detail::make_tbb_copy_pixels_fun(
                color_converted_view<typename V2::value_type>(src, _cc), dst),
...
GIL_FORCEINLINE void tbb_copy_and_convert_pixels(
    const V1& src,
    const V2& dst,
    CC        cc)
{
