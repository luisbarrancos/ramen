...
template<>
struct is_class<half> : public false_type
{
};
...
// channel conversion
template<>
struct channel_converter_unsigned<bits16f, bits8> : public std::unary_function<bits16f, bits8>
{
    bits8 operator()(bits16f x) const { return static_cast<bits8>(half(x) * 255.0f + 0.5f); }
};

template<>
struct channel_converter_unsigned<bits8, bits16f> : public std::unary_function<bits8, bits16f>
{
    bits16f operator()(bits8 x) const { return static_cast<bits16f>(half(x) / 255.0f); }
};

template<>
struct channel_converter_unsigned<bits16f, bits16> : public std::unary_function<bits16f, bits16>
{
    bits16 operator()(bits16f x) const { return static_cast<bits16>(half(x) * 65535.0f + 0.5f); }
};

template<>
struct channel_converter_unsigned<bits16, bits16f> : public std::unary_function<bits16, bits16f>
{
...
// floating point
template<>
struct channel_converter_unsigned<bits32f, bits16f> : public std::unary_function<bits32f, bits16f>
{
    bits16f operator()(bits32f x) const { return bits16f(half(x)); }
};

template<>
struct channel_converter_unsigned<bits16f, bits32f> : public std::unary_function<bits16f, bits32f>
{
    bits32f operator()(bits16f x) const { return bits32f(half(x)); }
};

template<>
struct channel_multiplier_unsigned<bits16f> : public std::binary_function<bits16f, bits16f, bits16f>
{
