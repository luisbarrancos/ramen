                              ...
                              float             pixel_scale)
{
    ...
    return pick_xy_axes(
        p, c, Imath::V2f(cs, ss) * xsize, Imath::V2f(ss, -cs) * ysize, xscale, pixel_scale);
}
                         ...
                         bool                pick_edges)
{
    ...
    if (pick_edges)
    {
        if (inside_pick_distance(
                p, Imath::V2f(box.min.x, box.min.y), Imath::V2f(box.max.x, box.min.y), pixel_scale))
            return box_top_edge_picked;

        if (inside_pick_distance(
                p, Imath::V2f(box.min.x, box.min.y), Imath::V2f(box.min.x, box.max.y), pixel_scale))
            return box_left_edge_picked;

        if (inside_pick_distance(
                p, Imath::V2f(box.min.x, box.max.y), Imath::V2f(box.max.x, box.max.y), pixel_scale))
            return box_bot_edge_picked;

        if (inside_pick_distance(
                p, Imath::V2f(box.max.x, box.min.y), Imath::V2f(box.max.x, box.max.y), pixel_scale))
