...
class RAMEN_API manipulator_t : boost::noncopyable
{
  ...
  public:  // These are for the python bindings, do not use.
    ...
    bool __do_key_press_event(const ui::key_press_event_t& event)
    {
        return manipulator_t::do_key_press_event(event);
    }
    void __do_key_release_event(const ui::key_release_event_t& event)
    {
        manipulator_t::do_key_release_event(event);
    }
    void __do_mouse_enter_event(const ui::mouse_enter_event_t& event)
    {
        manipulator_t::do_mouse_enter_event(event);
    }
    void __do_mouse_leave_event(const ui::mouse_leave_event_t& event)
    {
        manipulator_t::do_mouse_leave_event(event);
    }
    bool __do_mouse_press_event(const ui::mouse_press_event_t& event)
    {
        return manipulator_t::do_mouse_press_event(event);
    }
    void __do_mouse_move_event(const ui::mouse_move_event_t& event)
    {
        manipulator_t::do_mouse_move_event(event);
    }
    void __do_mouse_drag_event(const ui::mouse_drag_event_t& event)
    {
        manipulator_t::do_mouse_drag_event(event);
    }
    void __do_mouse_release_event(const ui::mouse_release_event_t& event)
    {
        manipulator_t::do_mouse_release_event(event);
    }
};
