...
void transform2_manipulator_t::do_draw_overlay(const ui::paint_event_t& event) const
{
    ...
    Imath::M33d m(
        param_->matrix_at_frame(app().document().composition().frame(), node->aspect_ratio()));
...
bool transform2_manipulator_t::do_key_press_event(const ui::key_press_event_t& event)
{
    return false;
}
