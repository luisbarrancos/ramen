...
namespace ramen
{
transform2_manipulator_t::transform2_manipulator_t(param_t* param)
  : manipulator_t()
{
...
void transform2_manipulator_t::do_draw_overlay(
    const ui::paint_event_t& event) const
{
    const image_node_t* node = dynamic_cast<const image_node_t*>(parent());
    assert(node);

    Imath::M33d m(param_->matrix_at_frame(
        app().document().composition().frame(), node->aspect_ratio()));

    // adjust for the aspect factor
    Imath::M33d mm =
        m * Imath::M33d().setScale(Imath::V2d(event.aspect_ratio, 1));
...
bool transform2_manipulator_t::do_key_press_event(
    const ui::key_press_event_t& event)
{
    return false;
}
void transform2_manipulator_t::do_key_release_event(
    const ui::key_release_event_t& event)
{
}

bool transform2_manipulator_t::do_mouse_press_event(
    const ui::mouse_press_event_t& event)
{
    ...
    Imath::M33d m(param_->matrix_at_frame(
        app().document().composition().frame(), aspect));
...
void transform2_manipulator_t::do_mouse_drag_event(
    const ui::mouse_drag_event_t& event)
{
    assert(drag_axes_ || drag_center_);
...
void transform2_manipulator_t::do_mouse_release_event(
