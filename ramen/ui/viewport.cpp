...
Imath::V2f viewport_t::screen_to_world(const Imath::V2i& p) const
{
    ...
    return Imath::V2f(
        ((p.x - device_.min.x) / zoom_x()) + world_.min.x,
...
{
    return Imath::Box2f(
        screen_to_world(Imath::V2i(b.min.x, b.min.y)),
...
{
    return Imath::Box2i(
        world_to_screen(Imath::V2f(b.min.x, b.min.y)),
...
{
    world_ = Imath::Box2f(
        Imath::V2i(device_.min.x, device_.min.y),
...
{
    resize(Imath::Box2i(Imath::V2i(0, 0), Imath::V2i(w - 1, h - 1)));
}

void viewport_t::scroll(const Imath::V2i& inc)
{
    if (y_down_)
        world_ = ImathExt::offsetBy(
            world_, Imath::V2f(inc.x / zoom_x(), -inc.y / zoom_y()));
    else
        world_ = ImathExt::offsetBy(
            world_, Imath::V2f(inc.x / zoom_x(), inc.y / zoom_y()));
}
