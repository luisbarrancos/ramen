...
namespace viewer
{
viewer_strategy_t::viewer_strategy_t(viewer_context_t* p)
  : parent_(p)
  , active_(false)
{
    assert(p != 0);
...
void viewer_strategy_t::mouse_move_event(QMouseEvent* event)
{
    event->ignore();
}
void viewer_strategy_t::mouse_press_event(QMouseEvent* event)
{
    event->ignore();
}
void viewer_strategy_t::mouse_release_event(QMouseEvent* event)
{
    event->ignore();
}
...
void viewer_strategy_t::pick_colors_in_box(
    const Imath::Box2i&                        b,
...
default_viewer_strategy_t::default_viewer_strategy_t(viewer_context_t* p)
  : viewer_strategy_t(p)
{
}

bool default_viewer_strategy_t::can_display_node(node_t* n) const
{
    return true;
}

void default_viewer_strategy_t::set_active_node(node_t* n)
{
    parent()->update();
}
void default_viewer_strategy_t::set_context_node(node_t* n)
{
    parent()->update();
}
