...
namespace viewer
{
tiled_image_strategy_t::tile_t::tile_t(
...
void tiled_image_strategy_t::tile_t::update_texture(
    const Imath::Box2i& area,
    char*               ptr,
    std::size_t         rowsize)
{
    ...
    {
        gl_tex_subimage2d(
            GL_TEXTURE_2D, 0, 0, j, width, 1, GL_RGBA, GL_FLOAT, p);
        p -= rowsize;
        ++j;
    }
}

char* tiled_image_strategy_t::tile_t::pixel_ptr(
    const image::buffer_t& pixels,
    int                    x,
    int                    y)
{
    ...
    return (char*) &(pixels.const_rgba_view()(
        x - pixels.bounds().min.x, y - pixels.bounds().min.y)[0]);
}

std::size_t tiled_image_strategy_t::tile_t::rowbytes(
    const image::buffer_t& pixels)
{
...
void tiled_image_strategy_t::tile_t::draw() const
{
    gl_bind_texture(GL_TEXTURE_2D, texture_id_);
    gl_textured_quad(
        area_.min.x, area_.min.y, area_.max.x + 1, area_.max.y + 1);
}

tiled_image_strategy_t::tiled_image_strategy_t(
    const image::buffer_t& pixels,
...
bool tiled_image_strategy_t::update_pixels(
    const image::buffer_t& pixels,
    const Imath::Box2i&    display_window,
    const Imath::Box2i&    data_window)
{
    // if( pixels.width() == width() && pixels.height() == height())
    if (data_window.size().x + 1 == width() &&
        data_window.size().y + 1 == height())
    {
        pixels_ = pixels;
        ...
        int xtiles =
            std::ceil((double) (data_window_.size().x + 1) / tile_size());
        int ytiles =
            std::ceil((double) (data_window_.size().y + 1) / tile_size());
...
boost::optional<Imath::Color4f> tiled_image_strategy_t::color_at(
    const Imath::V2i& p) const
{
    ...
    image::pixel_t px(pixels_.const_rgba_view()(
        p.x - data_window().min.x, p.y - data_window().min.y));
