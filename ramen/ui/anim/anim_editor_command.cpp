...
namespace undo
{
anim_editor_command_t::anim_editor_command_t(
    node_t*                                   node,
...
void anim_editor_command_t::swap_curves()
{
    for (iterator it(curves_.begin()); it != curves_.end(); ++it)
        anim::swap(
            const_cast<anim::any_curve_ptr_t&>(it->first->curve().get()),
            it->second);
}
...
void anim_editor_command_t::notify_nodes()
{
    ...
    // breadth_first_multiple_outputs_search( nodes_, boost::bind(
    // &node_t::do_notify, _1), true);
    (*nodes_.begin())->notify();
}
