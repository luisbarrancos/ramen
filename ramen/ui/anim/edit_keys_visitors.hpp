...
{
    can_drag_keys_visitor( const math::vector2f_t& offset);
...
{
    drag_keys_visitor( const math::vector2f_t& offset, bool snap_frames);
    void operator()( anim::float_curve_t *c);
...
{
    can_insert_keyframe_visitor( const anim_curves_view_t& view,
...
{
    insert_keyframe_visitor( const anim_curves_view_t& view, const math::point2i_t& p, bool snap);
...
{
    smooth_selected_keys_visitor( float filter_size,
                                  bool resample) : filter_size_( filter_size), resample_( resample) {}
...
{
    high_pass_selected_keys_visitor( float filter_size,
                                     bool resample) : filter_size_( filter_size), resample_( resample) {}
