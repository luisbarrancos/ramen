...
void drag_left_tangent( const anim_curves_view_t& view,
                        ...
                        const math::point2i_t& loc)
{
    float v = c->relative_to_absolute( key.value());
    math::point2i_t p( view.world_to_screen( math::point2f_t( key.time(), v)));
    math::point2f_t q( loc.x, loc.y);
    float slope;
    ...
    {
        math::vector2f_t v( q.x - p.x, q.y - p.y);
        v.x /= -view.time_scale();
...
drag_tangents_visitor::drag_tangents_visitor( const anim_curves_view_t& view, int index, bool left,
                                              const math::point2i_t& p, bool break_tangent) : view_( view)
{
