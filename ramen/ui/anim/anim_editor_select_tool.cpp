...
void anim_editor_select_tool_t::mouse_press_event( anim_curves_view_t& view, QMouseEvent *event)
{
    ...
    math::point2i_t p( push_x_, push_y_);
...
void anim_editor_select_tool_t::mouse_drag_event( anim_curves_view_t& view, QMouseEvent *event)
{
    ...
    if( drag_tangents_mode_)
    {
        drag_tangents_visitor v( view, key_index_, left_,
                                 math::point2i_t( event->x(), event->y()),
                                 break_tangents_);
    ...
    else
    {
        if( drag_keyframes_mode_)
        {
            ...
            {
                int dx = abs( event->x() - push_x_);
                int dy = abs( event->y() - push_y_);
            ...
            math::vector2f_t d(  ( event->x() - push_x_) / view.time_scale(),
...
void anim_editor_select_tool_t::mouse_release_event( anim_curves_view_t& view, QMouseEvent *event)
{
    ...
    else
    {
        ...
        {
            math::box2i_t area;
            area.extend_by( math::point2i_t( push_x_, push_y_));
            area.extend_by( math::point2i_t( last_x_, last_y_));
