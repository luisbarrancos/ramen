...
void param_spinbox_t::contextMenuEvent( QContextMenuEvent *event)
{
    ...
    if( !param_.is_static())
    {
        ...
        if( has_anim)
        {
            float frame = app().document().composition_node().frame();
...
void param_spinbox_t::textChanged()
{
    if( isModified())
    {
        ...
        boost::optional<double> result = calc()( core::string8_t( s.c_str()));
...
void param_spinbox_t::delete_key()
{
    ...
    aparam_->delete_key( comp_index_, app().document().composition_node().frame());
}
