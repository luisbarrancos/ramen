...
QToolBar* main_window_t::create_time_toolbar()
{
    ...
    time_slider_->setSizePolicy(QSizePolicy::Expanding,
                                time_slider_->sizePolicy().verticalPolicy());
...
void main_window_t::import_composition()
{
    ...
    if (!(fname.isEmpty()))
    {
        ...
        {
            QMessageBox::warning(
                this, "Error opening document", "Old version or corrupted composition");
        }
...
{
    /*
    QString fname = QFileDialog::getSaveFileName( 0, "Save Composition As", QString::null,
    file_dialog_extension(), 0, QFileDialog::DontUseNativeDialog);
...
void main_window_t::delete_nodes()
{
    ...
    if (autoconnect)
    {
        ...
        std::stable_sort(
            edges_to_add.begin(), edges_to_add.end(), &undo::delete_command_t::edge_less);
        std::unique(
            edges_to_add.begin(), edges_to_add.end(), &undo::delete_command_t::edge_compare);
...
void main_window_t::render_flipbook()
{
    ...
    render_flipbook_dialog_t::instance().mblur_extra_samples(); float mb_shutter_factor =
    render_flipbook_dialog_t::instance().mblur_shutter_factor();

            flipbook::flipbook_t *flip = flipbook::factory_t::instance().create(
    render_flipbook_dialog_t::instance().flipbook(), frame_rate, display_device, display_transform);

            if( flip)
            {
                if( flipbook::render_flipbook( flip, n, start, end, proxy_level, subsample,
    mb_extra_samples, mb_shutter_factor)) flip->play();
            }
        }
    }
    */
...
void main_window_t::show_preferences_dialog() { preferences_dialog_t::instance().exec_dialog(); }
...
void main_window_t::create_node()
{
    ...
    {
        app().ui()->error(std::string("Couldn't create node ") + id + std::string(" ") + e.what());
        return;
    }
