  ...
  : QMainWindow()
{
    ...
    inspector_dock_->setAllowedAreas(
    ...
    anim_editor_dock_->setAllowedAreas(
    ...
    // Composition view
    {
        ...
        composition_dock_->setAllowedAreas(
            Qt::BottomDockWidgetArea | Qt::TopDockWidgetArea |
            Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
...
QToolBar* main_window_t::create_time_toolbar()
{
    ...
    time_slider_->setSizePolicy(
        QSizePolicy::Expanding, time_slider_->sizePolicy().verticalPolicy());
    connect(
        time_slider_,
        SIGNAL(start_frame_changed(int)),
        app().ui(),
        SLOT(set_start_frame(int)));
    connect(
        time_slider_,
        SIGNAL(end_frame_changed(int)),
        app().ui(),
        SLOT(set_end_frame(int)));
    connect(
        time_slider_,
        SIGNAL(time_changed(int)),
        app().ui(),
        SLOT(set_frame(int)));
...
void main_window_t::create_actions()
{
    ...
    connect(
        import_comp_, SIGNAL(triggered()), this, SLOT(import_composition()));
    ...
    connect(
        ...
        SIGNAL(triggered()),
        this,
        SLOT(show_composition_settings_dialog()));
    ...
    connect(
        comp_render_, SIGNAL(triggered()), this, SLOT(render_composition()));
    ...
    connect(
        project_web_, SIGNAL(triggered()), this, SLOT(go_to_project_website()));
...
void main_window_t::create_node_actions()
{
    ...
    for (const node_metaclass_t& mclass :
         node_factory_t::instance().registered_nodes())
    {
        if (mclass.ui_visible &&
            node_factory_t::instance().is_latest_version(mclass.id))
        {
...
void main_window_t::import_composition()
{
    ...
    if (!(fname.isEmpty()))
    {
        try
        {
            ...
            ramen::import_composition( p)); app().document().set_dirty( true);
            app().document().undo_stack().clear_all();
            ...
            if( !err.empty())
                multiline_alert_t::instance().show_alert( "Errors during file
            open", err);
            */
        }
...
{
    /*
    QString fname = QFileDialog::getSaveFileName( 0, "Save Composition As",
    QString::null, file_dialog_extension(), 0,
    QFileDialog::DontUseNativeDialog);
...
{
    std::auto_ptr<undo::ignore_nodes_command_t> c(
        new undo::ignore_nodes_command_t());
    ...
    if (autoconnect)
    {
        ...
        for (node_t& n : app().document().composition().nodes())
        {
            if (n.selected())
            {
                ...
                if (src)
                {
                    breadth_first_out_edges_apply(
                        n,
                        boost::bind(
                            &undo::delete_command_t::add_candidate_edge,
                            _1,
                            src,
...
void main_window_t::clear_cache() { app().memory_manager().clear_caches(); }
...
{
    bool any_output =
        (render::count_output_nodes(app().document().composition()) != 0);
    ...
    bool any_output_selected =
        (render::count_output_nodes(app().document().composition(), true) != 0);

    render_composition_dialog_t::instance().set_any_output_selected(
        any_output_selected);
    ...
    if (result == QDialog::Accepted)
    {
        ...
        ui::render_composition(
            app().document().composition(),
            start,
            end,
            render_composition_dialog_t::instance().proxy_level(),
...
void main_window_t::render_flipbook()
{
    ...
    if( result == QDialog::Accepted)
    {
        if( image_node_t *n = dynamic_cast<image_node_t*>(
    app().document().composition().selected_node()))
        {
            int frame_rate = app().document().composition().frame_rate();
            std::string display_device =
    render_flipbook_dialog_t::instance().display_device(); std::string
    display_transform =
    render_flipbook_dialog_t::instance().display_transform(); int start	=
    render_flipbook_dialog_t::instance().start_frame(); int end	=
    render_flipbook_dialog_t::instance().end_frame(); int subsample =
    render_flipbook_dialog_t::instance().resolution(); int proxy_level =
    render_flipbook_dialog_t::instance().proxy_level(); int mb_extra_samples =
    render_flipbook_dialog_t::instance().mblur_extra_samples(); float
    mb_shutter_factor =
    render_flipbook_dialog_t::instance().mblur_shutter_factor();

            flipbook::flipbook_t *flip = flipbook::factory_t::instance().create(
    render_flipbook_dialog_t::instance().flipbook(), frame_rate, display_device,
    display_transform);

            if( flip)
            {
                if( flipbook::render_flipbook( flip, n, start, end, proxy_level,
    subsample, mb_extra_samples, mb_shutter_factor)) flip->play();
            }
        }
    }
    */
...
void main_window_t::create_node()
{
    ...
    {
        app().ui()->error(
            std::string("Couldn't create node ") + id + std::string(" ") +
            e.what());
        return;
    }
...
void main_window_t::show_about_box()
{
    ...
    msgBox.setText(QString("<html><p style=\" font-size:16pt;"
                           "font-weight:bold;\">%1</p></html>")
                       .arg(RAMEN_NAME_FULL_VERSION_STR));
...
void main_window_t::go_to_project_website()
{
    QDesktopServices::openUrl(
        QUrl("http://ramencomp.blogspot.com/", QUrl::TolerantMode));
}
...
void main_window_t::update()
{
    ...
    update_menus();
    time_slider_->update(
        app().document().composition().start_frame(),
...
void main_window_t::update_recent_files_menu(const boost::filesystem::path& p)
{
    ...
    int num_recent_files =
        std::min(files.size(), (int) max_recently_opened_files);
...
void main_window_t::update_menus()
{
    ...
    else
    {
        undo_->setText(
            QString("Undo ") +
            app().document().undo_stack().last_undo_command().name().c_str());
    ...
    else
    {
        redo_->setText(
            QString("Redo ") +
            app().document().undo_stack().last_redo_command().name().c_str());
