...
namespace ui
{
input_location_node_visitor::input_location_node_visitor(int i)
: index(i)
{
}
void input_location_node_visitor::visit(node_t* n)
{
    assert(index >= 0 && index < n->num_inputs());
    loc = generic_input_location(n, index);
}

output_location_node_visitor::output_location_node_visitor(int i)
: index(i)
{
}
...
draw_node_visitor::draw_node_visitor(QPainter& painter)
: painter_(painter)
{
}
void draw_node_visitor::visit(node_t* n) { draw_generic_node(painter_, n); }

draw_edges_visitor::draw_edges_visitor(const composition_view_t& view, QPainter& painter)
: view_(view)
, painter_(painter)
{
}
...
void pick_node_visitor::visit(node_t* n) { pick_generic_node(n, p_, view_, result_); }

box_pick_node_visitor::box_pick_node_visitor(const Imath::Box2f& box)
: box_(box)
, result(false)
{
}
void box_pick_node_visitor::visit(node_t* n) { result = box_pick_generic_node(n, box_); }

pick_edge_visitor::pick_edge_visitor(const composition_view_t& view, const Imath::V2f& p)
: view_(view)
, src(0)
, dst(0)
, port(-1)
