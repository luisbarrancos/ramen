...
void image_seq_reader_t::get_sequence_info()
{
    if (is_sequence())
    {
        for (int i = start_frame(); i <= end_frame(); ++i)
        {
            try
            {
                core::auto_ptr_t<imageio::reader_t> r(
                    imageio::factory_t::instance().reader_for_image(seq_(i)));
    ...
    else
    {
        ...
        {
            core::auto_ptr_t<imageio::reader_t> r(
                imageio::factory_t::instance().reader_for_image(seq_()));
            reader_.reset(r.release());
        }
        catch (...)
        {
...
void image_seq_reader_t::create_reader()
{
    ...
    {
        core::auto_ptr_t<imageio::reader_t> r(
            imageio::factory_t::instance().reader_for_image(seq_(frame_)));
        reader_.reset(r.release());
    }
    catch (imageio::exception& e)
    {
