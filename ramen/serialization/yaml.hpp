...
template <class T> void operator>>(const YAML::Node& in, Imath::Vec2<T>& v)
{
    in[0] >> v.x;
    in[1] >> v.y;
}
...
template <class T> void operator>>(const YAML::Node& in, Imath::Vec3<T>& v)
{
    in[0] >> v.x;
    in[1] >> v.y;
    in[2] >> v.z;
...
template <class T> void operator>>(const YAML::Node& in, Imath::Color3<T>& c)
{
    in[0] >> c.x;
    in[1] >> c.y;
    in[2] >> c.z;
...
template <class T> void operator>>(const YAML::Node& in, Imath::Color4<T>& c)
{
    in[0] >> c.r;
    in[1] >> c.g;
    in[2] >> c.b;
...
{
    out << YAML::Flow << YAML::BeginSeq << c.r << c.g << c.b << c.a
        << YAML::EndSeq;
    return out;
}
