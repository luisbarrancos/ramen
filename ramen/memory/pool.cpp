...
struct pool_t::implementation_t
{
    ...
    struct mem_block_t : public bi::list_base_hook<bi::link_mode<bi::safe_link>>
    {
        ...
        bool merge(mem_block_t& other)
        {
            ...
            assert(p < q && "merge: list posibly not sorted");
    ...
    iterator find_best_block(std::size_t s)
    {
        ...
        {
            assert(it->free && "find best block: non free block found");
    ...
    {
        // preconditions
        assert(it != end() && "split block: it == end()");
        assert(it->free && "split block: trying to split a used block");
        ...
        // postconditions
        assert(list_is_sorted() && "split block: list not sorted");
        assert(block_in_pool(&*it) && "split block: block not in pool");
        assert(block_in_pool(new_block) && "split block: block not in pool");
    }
...
{
    assert(!m_pimpl.get() && "image_mem_pool_t::init already called");
    m_pimpl.reset(new implementation_t(size));
    allocated_ = 0;
}

std::size_t pool_t::pool_size() const
{
    assert(m_pimpl.get() && "image_mem_pool_t::init not called");
    return m_pimpl->pool_size;
}
...
unsigned char* pool_t::allocate(std::size_t& size)
{
    ...
    unsigned char* p = m_pimpl->allocate(size);
