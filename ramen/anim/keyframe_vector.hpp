...
namespace
{
template <class T> struct keyframe_less
{
    ...
    template <class K> bool operator()(const K& a, T b) const
    {
        return a.time() < b;
    }

    // MSVC debug builds requires the next overloads
    template <class K> bool operator()(T a, const K& b) const
    {
        return a < b.time();
    }
...
template <class K> class keyframe_vector_t
{
  public:
    typedef K                      key_type;
    ...
    typedef typename std::vector<key_type>::const_reverse_iterator
    ...
    const_iterator lower_bound(time_type time) const
    {
        return std::lower_bound(
            begin(), end(), time, keyframe_less<time_type>());
    }
