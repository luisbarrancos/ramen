...
{
    return Imath::clamp(do_evaluate(time), min_, max_);
}
...
void float_curve_t::recalc_tangents_and_coefficients(iterator it)
{
    ...
    if (ptr[1])
        ptr[1]->calc_tangents(ptr[0], ptr[2]);
    if (ptr[2])
        ptr[2]->calc_tangents(ptr[1], ptr[3]);
    if (ptr[3])
        ptr[3]->calc_tangents(ptr[2], ptr[4]);

    if (ptr[0] && ptr[1])
        ptr[0]->calc_cubic_coefficients(*ptr[1]);
    if (ptr[1] && ptr[2])
        ptr[1]->calc_cubic_coefficients(*ptr[2]);
    if (ptr[2] && ptr[3])
        ptr[2]->calc_cubic_coefficients(*ptr[3]);
    if (ptr[3] && ptr[4])
        ptr[3]->calc_cubic_coefficients(*ptr[4]);
}
...
{
    out.begin_map();
    out << YAML::Key << "extrapolation" << YAML::Value
        << extrapolation_method_to_string(extrapolation());
