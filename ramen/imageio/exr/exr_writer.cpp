...
{
    return Imath::Box2i(
        Imath::V2i(box.min.x, box.min.y), Imath::V2i(box.max.x, box.max.y));
}

}  // namespace

void exr_writer_t::do_write_image(
    ...
    const core::dictionary_t&        params) const
{
    ...
    Imath::Box2i display_window =
        convert_box(core::get<math::box2i_t>(params, core::name_t("format")));
    Imath::Box2i data_window = display_window;
    {
        const math::box2i_t* tmp =
            core::get<math::box2i_t>(&params, core::name_t("bounds"));

        if (tmp)
            data_window = convert_box(*tmp);
    }

    assert(display_window.intersects(data_window));
    ...
    image::const_image_view_t subview(boost::gil::subimage_view(
        view,
        data_window.min.x - display_window.min.x,
        data_window.min.y - display_window.min.y,
        data_window.size().x + 1,
        data_window.size().y + 1));
    ...
    bool                             save_alpha) const
{
    ...
    char* ptr = (char*) boost::gil::interleaved_view_get_raw_data(
        boost::gil::view(img));
    std::size_t xstride = 4 * sizeof(half);
    std::size_t ystride = xstride * img.width();
    ptr = ptr - (header.dataWindow().min.y * ystride) -
          (header.dataWindow().min.x * xstride);

    frameBuffer.insert("R", Imf::Slice(Imf::HALF, ptr, xstride, ystride));
    ptr += sizeof(half);
    frameBuffer.insert("G", Imf::Slice(Imf::HALF, ptr, xstride, ystride));
    ptr += sizeof(half);
    frameBuffer.insert("B", Imf::Slice(Imf::HALF, ptr, xstride, ystride));
    ptr += sizeof(half);
...
void exr_writer_t::write_half_alpha(
    const boost::filesystem::path&   p,
    Imf::Header&                     header,
    const image::const_image_view_t& view) const
{
    ...
    boost::gil::copy_and_convert_pixels(
        boost::gil::nth_channel_view(view, 3), boost::gil::view(img));
    ...
    bool                             save_alpha) const
{
    ...
    frameBuffer.insert("R", Imf::Slice(Imf::FLOAT, ptr, xstride, ystride));
    ptr += sizeof(float);
    frameBuffer.insert("G", Imf::Slice(Imf::FLOAT, ptr, xstride, ystride));
    ptr += sizeof(float);
    frameBuffer.insert("B", Imf::Slice(Imf::FLOAT, ptr, xstride, ystride));
    ptr += sizeof(float);
...
void exr_writer_t::write_float_alpha(
    const boost::filesystem::path&   p,
    Imf::Header&                     header,
    const image::const_image_view_t& view) const
{
