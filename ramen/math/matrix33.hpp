...
class matrix33_t
  : boost::
        multipliable<matrix33_t<T>, boost::equality_comparable<matrix33_t<T>>>
{
  public:
    ...
    {
        return matrix33_t<T>(1, 0, 0, 0, 1, 0, 0, 0, 1);
    }

    static matrix33_t<T> zero()
    {
        return matrix33_t<T>(0, 0, 0, 0, 0, 0, 0, 0, 0);
    }
...
{
    return vector2_t<S>(
        v.x * m(0, 0) + v.y * m(1, 0), v.x * m(0, 1) + v.y * m(1, 1));
}
