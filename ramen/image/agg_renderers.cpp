...
namespace image
{
void agg_rgba32f_renderer_t::blend_hline(
    int x1, int y, int x2, const color_type& c, agg::cover_type cover)
{
    ...
    {
        get_color(*p, red_t())
            = (get_color(*p, red_t()) * inv_alpha) + (get_color(c, red_t()) * alpha);
        get_color(*p, green_t())
            = (get_color(*p, green_t()) * inv_alpha) + (get_color(c, green_t()) * alpha);
        get_color(*p, blue_t())
            = (get_color(*p, blue_t()) * inv_alpha) + (get_color(c, blue_t()) * alpha);
...
void agg_rgba32f_renderer_t::blend_solid_hspan(
