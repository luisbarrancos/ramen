    ...
    const matrix3_t&          inv_xform)
{
    ...
    generic::affine_warp<sse2::bilinear_sampler_t>(
        s, src_area, dst_area, dst, xform, inv_xform);
}

void affine_warp_catrom(
    const Imath::Box2i&       src_area,
    const const_image_view_t& src,
    const Imath::Box2i&       dst_area,
    const image_view_t&       dst,
    ...
    const matrix3_t&          inv_xform)
{
    ...
    generic::affine_warp<sse2::catrom_sampler_t>(
        s, src_area, dst_area, dst, xform, inv_xform);
}

// non-black
void affine_warp_bilinear_tile(
    const Imath::Box2i&       src_area,
    const const_image_view_t& src,
    const Imath::Box2i&       dst_area,
    const image_view_t&       dst,
    ...
    const matrix3_t&          inv_xform)
{
    ...
    generic::affine_warp_non_black_border_mode<
        generic::tile_sampler_t<sse2::bilinear_sampler_t>>(
        ts, dst_area, dst, xform, inv_xform);
}

void affine_warp_bilinear_mirror(
    const Imath::Box2i&       src_area,
    const const_image_view_t& src,
    const Imath::Box2i&       dst_area,
    const image_view_t&       dst,
    ...
    const matrix3_t&          inv_xform)
{
    ...
    generic::affine_warp_non_black_border_mode<
        generic::mirror_sampler_t<sse2::bilinear_sampler_t>>(
        ts, dst_area, dst, xform, inv_xform);
}
