...
namespace
{
template <class Sampler> class warp_fun
{
  public:
    warp_fun(
        const Sampler&         s,
...
template <class Sampler> class warp_and_derivs_fun
{
  public:
    warp_and_derivs_fun(
        const Sampler&         s,
    ...
    bool                      sequential)
{
    ...
    if (sequential)
        warp_sequential<sse2::bilinear_sampler_t>(
            s, dst_area, dst, wfun, uv_derivs);
    else
        warp<sse2::bilinear_sampler_t>(s, dst_area, dst, wfun, uv_derivs);
}

void warp_bilinear_tile(
    const Imath::Box2i&       src_area,
    const const_image_view_t& src,
    const Imath::Box2i&       dst_area,
    const image_view_t&       dst,
    ...
    bool                      sequential)
{
    ...
    if (sequential)
        ...
        warp<generic::tile_sampler_t<sse2::bilinear_sampler_t>>(
            ts, dst_area, dst, wfun, uv_derivs);
}

void warp_bilinear_mirror(
    const Imath::Box2i&       src_area,
    const const_image_view_t& src,
    const Imath::Box2i&       dst_area,
    const image_view_t&       dst,
    ...
    bool                      sequential)
{
    ...
    if (sequential)
        warp_sequential<sse2::catrom_sampler_t>(
            s, dst_area, dst, wfun, uv_derivs);
    else
