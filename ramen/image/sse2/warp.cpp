...
{
public:
    warp_fun(const Sampler&         s,
             const Imath::Box2i&    dst_area,
             const image_view_t&    dst,
             const warp_function_t& fun)
    : dst_(dst)
    , fun_(fun)
    , s_(s)
    {
...
{
public:
    warp_and_derivs_fun(const Sampler&         s,
                        const Imath::Box2i&    dst_area,
                        const image_view_t&    dst,
                        const warp_function_t& fun)
    : dst_(dst)
...
template<class Sampler>
void warp(const Sampler&         s,
          const Imath::Box2i&    dst_area,
          const image_view_t&    dst,
          const warp_function_t& wfun,
          bool                   derivs = false)
{
    if (derivs)
    {
        tbb::parallel_for(tbb::blocked_range<int>(dst_area.min.y, dst_area.max.y + 1),
                          warp_and_derivs_fun<Sampler>(s, dst_area, dst, wfun),
                          tbb::auto_partitioner());
    }
    else
    {
        tbb::parallel_for(tbb::blocked_range<int>(dst_area.min.y, dst_area.max.y + 1),
                          warp_fun<Sampler>(s, dst_area, dst, wfun),
                          tbb::auto_partitioner());
    }
}

template<class Sampler>
void warp_sequential(const Sampler&         s,
                     const Imath::Box2i&    dst_area,
                     const image_view_t&    dst,
                     const warp_function_t& wfun,
                     bool                   derivs = false)
...
void warp_bilinear(const Imath::Box2i&       src_area,
...
void warp_bilinear_tile(const Imath::Box2i&       src_area,
                        const const_image_view_t& src,
                        const Imath::Box2i&       dst_area,
                        const image_view_t&       dst,
                        const warp_function_t&    wfun,
                        ...
                        bool                      sequential)
{
    ...
    if (sequential)
        warp_sequential<generic::tile_sampler_t<sse2::bilinear_sampler_t>>(
            ts, dst_area, dst, wfun, uv_derivs);
    else
        warp<generic::tile_sampler_t<sse2::bilinear_sampler_t>>(ts, dst_area, dst, wfun, uv_derivs);
}

void warp_bilinear_mirror(const Imath::Box2i&       src_area,
                          const const_image_view_t& src,
                          const Imath::Box2i&       dst_area,
                          const image_view_t&       dst,
                          const warp_function_t&    wfun,
                          ...
                          bool                      sequential)
{
    ...
    if (sequential)
        warp_sequential<generic::mirror_sampler_t<sse2::bilinear_sampler_t>>(
            tm, dst_area, dst, wfun, uv_derivs);
    else
        warp<generic::mirror_sampler_t<sse2::bilinear_sampler_t>>(
            tm, dst_area, dst, wfun, uv_derivs);
}

void warp_bicubic(const Imath::Box2i&       src_area,
                  const const_image_view_t& src,
                  const Imath::Box2i&       dst_area,
                  const image_view_t&       dst,
                  const warp_function_t&    wfun,
