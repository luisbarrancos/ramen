...
struct box_blur_rgba_fn
{
    ...
    void operator()(const tbb::blocked_range<std::size_t>& r) const
    {
        ...
        __m128 inv_fractw = _mm_set_ps(
            1.0f - fradius, 1.0f - fradius, 1.0f - fradius, 1.0f - fradius);
        ...
        for (std::size_t y = r.begin(); y != r.end(); ++y)
        {
            ...
            __m128 p =
                _mm_load_ps(reinterpret_cast<const float*>(&(src_it[indx])));
    ...
    int                       iters)
{
    ...
    tbb::parallel_for(
        ...
        box_blur_rgba_fn(src, tmp, hradius),
        tbb::auto_partitioner());

    tbb::parallel_for(
        tbb::blocked_range<std::size_t>(0, tmp.height()),
        box_blur_rgba_fn(tmp, dst, vradius),
        tbb::auto_partitioner());
    ...
    for (int i = 1; i < iters; ++i)
    {
        tbb::parallel_for(
            ...
            box_blur_rgba_fn(dst, tmp, hradius),
            tbb::auto_partitioner());
