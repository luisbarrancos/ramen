...
namespace detail
{
template <class Sampler> class affine_warp_fn
{
  public:
    affine_warp_fn(
        const Sampler&      s,
    ...
    const matrix3_t&    inv_xform)
{
    ...
    tbb::parallel_for(
        tbb::blocked_range<int>(active_area.min.y, active_area.max.y + 1),
        detail::affine_warp_fn<Sampler>(
            s, dst_area, dst, active_area, inv_xform),
        tbb::auto_partitioner());
}

template <class Sampler>
void affine_warp_non_black_border_mode(
    const Sampler&      s,
    const Imath::Box2i& dst_area,
    const image_view_t& dst,
    const matrix3_t&    xform,
