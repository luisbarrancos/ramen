...
{
    return Imath::V2i(
        Imath::Math<float>::floor(resize_coord(p.x, center.x, sx)),
...
{
    Imath::V2i p(
        Imath::Math<float>::floor((box.min.x - center.x) / sx) + center.x,
        Imath::Math<float>::floor((box.min.y - center.y) / sy) + center.y);

    Imath::V2i q(
        Imath::Math<float>::floor((box.max.x - center.x) / sx) + center.x,
