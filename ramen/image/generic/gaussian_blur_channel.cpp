...
{
    gaussian_blur_gray_fn(const ConstGrayView& src, const GrayView& dst, float* kernel, int size)
    : src_(src)
    , dst_(dst)
    {
        kernel_ = kernel;
...
template<class ConstGrayView, class GrayView>
gaussian_blur_gray_fn<ConstGrayView, GrayView> make_gaussian_blur_gray_fn(const ConstGrayView& src,
                                                                          const GrayView&      dst,
                                                                          float* kernel,
                                                                          int    size)
{
...
void do_gaussian_blur_gray(
    const ConstGrayView& src, const TmpView& tmp, const GrayView& dst, float stddevx, float stddevy)
{
    // create kernel here
