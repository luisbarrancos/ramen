...
double V2SquaredLength(a) Vector2* a;
{
    return ((a->x * a->x) + (a->y * a->y));
}

/* returns length of input vector */
double V2Length(a) Vector2* a;
{
    return (sqrt(V2SquaredLength(a)));
}

/* negates the input vector and returns it */
Vector2* V2Negate(v) Vector2* v;
{
    v->x = -v->x;
    v->y = -v->y;
    return (v);
...
Vector2* V2Normalize(v) Vector2* v;
{
...
Vector2 *V2Add(a, b, c) Vector2 *a, *b, *c;
{
    c->x = a->x + b->x;
    c->y = a->y + b->y;
    return (c);
...
Vector2 *V2Sub(a, b, c) Vector2 *a, *b, *c;
{
    c->x = a->x - b->x;
    c->y = a->y - b->y;
    return (c);
...
Vector2 *V2Lerp(lo, hi, alpha, result) Vector2 *lo, *hi, *result;
double   alpha;
{
...
Vector2 *V2Mul(a, b, result) Vector2 *a, *b, *result;
{
    result->x = a->x * b->x;
    result->y = a->y * b->y;
    return (result);
...
double V2DistanceBetween2Points(a, b) Point2 *a, *b;
{
    double dx = a->x - b->x;
    double dy = a->y - b->y;
...
Vector2 *V2MakePerpendicular(a, ap) Vector2 *a, *ap;
{
    ap->x = -a->y;
    ap->y = a->x;
    return (ap);
...
Point2*  V2MulPointByMatrix(p, m) Point2* p;
Matrix3* m;
{
    double w;
    Point2 ptmp;
...
Matrix3 *V2MatMul(a, b, c) Matrix3 *a, *b, *c;
{
    int i, j, k;
    for (i = 0; i < 3; i++)
    {
...
/* returns squared length of input vector */
double V3SquaredLength(a) Vector3* a;
{
    return ((a->x * a->x) + (a->y * a->y) + (a->z * a->z));
}

/* returns length of input vector */
double V3Length(a) Vector3* a;
{
    return (sqrt(V3SquaredLength(a)));
}

/* negates the input vector and returns it */
Vector3* V3Negate(v) Vector3* v;
{
    v->x = -v->x;
    v->y = -v->y;
    v->z = -v->z;
...
/* normalizes the input vector and returns it */
Vector3* V3Normalize(v) Vector3* v;
{
...
/* scales the input vector to the new length and returns it */
Vector3* V3Scale(v, newlen) Vector3* v;
double   newlen;
{
    double len = V3Length(v);
    if (len != 0.0)
...
/* return vector difference c = a-b */
Vector3 *V3Sub(a, b, c) Vector3 *a, *b, *c;
{
    c->x = a->x - b->x;
    c->y = a->y - b->y;
    c->z = a->z - b->z;
...
/* and return the resulting vector. */
/* When alpha=0, result=lo.  When alpha=1, result=hi. */
Vector3 *V3Lerp(lo, hi, alpha, result) Vector3 *lo, *hi, *result;
double   alpha;
{
    result->x = LERP(alpha, lo->x, hi->x);
    result->y = LERP(alpha, lo->y, hi->y);
...
/* make a linear combination of two vectors and return the result. */
/* result = (a * ascl) + (b * bscl) */
Vector3 *V3Combine(a, b, result, ascl, bscl) Vector3 *a, *b, *result;
double   ascl, bscl;
{
    result->x = (ascl * a->x) + (bscl * b->x);
    result->y = (ascl * a->y) + (bscl * b->y);
...
/* return the distance between two points */
double V3DistanceBetween2Points(a, b) Point3 *a, *b;
{
    double dx = a->x - b->x;
    double dy = a->y - b->y;
    double dz = a->z - b->z;
...
Vector3 *V3Cross(a, b, c) Vector3 *a, *b, *c;
{
    c->x = (a->y * b->z) - (a->z * b->y);
    c->y = (a->z * b->x) - (a->x * b->z);
    c->z = (a->x * b->y) - (a->y * b->x);
...
/* multiply together matrices c = ab */
/* note that c must not point to either of the input matrices */
Matrix4 *V3MatMul(a, b, c) Matrix4 *a, *b, *c;
{
    int i, j, k;
    for (i = 0; i < 4; i++)
    {
...
int quadraticRoots(a, b, c, roots) double a, b, c, *roots;
{
    double d, q;
    int    count = 0;
    d            = (b * b) - (4 * a * c);
...
double RegulaFalsi(f, left, right) double (*f)(), left, right;
{
...
double NewtonRaphson(f, df, x) double (*f)(), (*df)(), x;
{
    double d = (*df)(x);
    if (d != 0.0)
        return (x - ((*f)(x) / d));
...
/* Based on Blinn */
double findroot(left, right, tolerance, f, df) double left, right, tolerance;
double (*f)(), (*df)();
{
    double newx = left;
