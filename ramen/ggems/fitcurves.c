 ...
 */
void   FitCurve(d, nPts, error) Point2* d; /*  Array of digitized points	*/
...
static double B2(u) double u;
{
    double tmp = 1.0 - u;
    return (3 * u * u * tmp);
}

static double B3(u) double u;
{
    return (u * u * u);
}
...
static Vector2 V2AddII(a, b) Vector2 a, b;
{
    Vector2 c;
    c.x = a.x + b.x;
    c.y = a.y + b.y;
...
static Vector2 V2ScaleIII(v, s) Vector2 v;
double         s;
{
    Vector2 result;
    result.x = v.x * s;
...
static Vector2 V2SubII(a, b) Vector2 a, b;
{
    Vector2 c;
    c.x = a.x - b.x;
    c.y = a.y - b.y;
