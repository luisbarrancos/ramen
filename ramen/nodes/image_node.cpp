...
void image_node_t::calc_format(const render::context_t& context)
{
    ...
    assert(proxy_scale().x != 0.0f && proxy_scale().y != 0.0f);
}
...
bool image_node_t::read_image_from_cache(const render::context_t& context)
{
    ...
    boost::optional<image::buffer_t> cached(
        app().memory_manager().find_in_cache(digest(), defined()));
...
{
    return image_.const_rgba_subimage_view(defined());
}
...
void image_node_t::recursive_process(const render::context_t& context)
{
    if (interacting())
    {
        assert(context.mode == render::interface_render);
    }
