...
namespace
{
// this colorspace is also called lab, but it's not the same as the commonly
// used lab
struct convert_rgb_to_lab_fun
{
    image::pixel_t operator()(const image::pixel_t& p) const
    {
        ...
        return image::pixel_t(
            (0.57735f * l) + (0.57735f * m) + (0.57735f * s),
            (0.40825f * l) + (0.40825f * m) - (0.81650f * s),
            (0.70711f * l) - (0.70711f * m),
            alpha);
    }
};
...
struct convert_lab_to_rgb_fun
{
    image::pixel_t operator()(const image::pixel_t& p) const
    {
        ...
        return image::pixel_t(
            (4.4686f * l) - (3.5886f * m) + (0.1196f * s),
            -(1.2197f * l) + (2.3830f * m) - (0.1626f * s),
            (0.0585f * l) - (0.2610f * m) + (1.2056f * s),
            alpha);
    }
};

void cm_calc_mean_and_stddev(
    const image::const_image_view_t& img,
    Imath::V3f&                      mean,
    Imath::V3f&                      std)
{
    using namespace boost::gil;
    ...
    std = Imath::V3f(
        std::sqrt((double) var.x),
        std::sqrt((double) var.y),
        std::sqrt((double) var.z));
}

struct transfer_color_fun
{
    transfer_color_fun(
        const Imath::V3f& src_mean,
        const Imath::V3f& src_dev,
        const Imath::V3f& dst_mean,
        const Imath::V3f& dst_dev)
    {
        src_mean_ = src_mean;
        dst_mean_ = dst_mean;
    ...
    image::pixel_t operator()(const image::pixel_t& p) const
    {
        ...
        return image::pixel_t(
            ((r - src_mean_.x) * gain_.x) + dst_mean_.x,
            ((g - src_mean_.y) * gain_.y) + dst_mean_.y,
            ((b - src_mean_.z) * gain_.z) + dst_mean_.z,
            a);
    }
...
color_match_node_t::color_match_node_t()
{
    ...
    add_input_plug(
        "source",
        true,
        ui::palette_t::instance().color("back plug"),
        "Source Color");
    add_output_plug();
}

void color_match_node_t::do_calc_inputs_interest(
    const render::context_t& context)
{
    // we need all the image of the second input
    input_as<image_node_t>(1)->add_interest(
        input_as<image_node_t>(1)->format());

    if (input(2))
    {
        input_as<image_node_t>(0)->add_interest(interest());
        input_as<image_node_t>(2)->add_interest(
            input_as<image_node_t>(2)->format());
    }
    else
        input_as<image_node_t>(0)->add_interest(
            input_as<image_node_t>(0)->format());
}
...
void color_match_node_t::do_process(const render::context_t& context)
{
    ...
    Imath::Box2i area =
        ImathExt::intersect(defined(), input_as<image_node_t>(0)->defined());
    ...
    // process input 1
    {
        image::const_image_view_t src_view(
            input_as<image_node_t>(1)->const_subimage_view(
                input_as<image_node_t>(1)->format()));
        image::image_t tmp(src_view.width(), src_view.height());
        boost::gil::tbb_transform_pixels(
            src_view, boost::gil::view(tmp), convert_rgb_to_lab_fun());
    ...
    {
        image::const_image_view_t src_view(
            input_as<image_node_t>(2)->const_subimage_view(
                input_as<image_node_t>(2)->format()));
        image::image_t tmp(src_view.width(), src_view.height());
        boost::gil::tbb_transform_pixels(
            src_view, boost::gil::view(tmp), convert_rgb_to_lab_fun());
    ...
    boost::gil::tbb_transform_pixels(
        input_as<image_node_t>(0)->const_subimage_view(area),
        subimage_view(area),
        convert_rgb_to_lab_fun());

    if (!input(2))
        cm_calc_mean_and_stddev(const_subimage_view(area), src_mean, src_dev);

    boost::gil::tbb_transform_pixels(
        const_subimage_view(area),
        subimage_view(area),
        transfer_color_fun(src_mean, src_dev, dst_mean, dst_dev));
    boost::gil::tbb_transform_pixels(
        const_subimage_view(area),
        subimage_view(area),
        convert_lab_to_rgb_fun());
}

// factory
node_t* create_color_match_node() { return new color_match_node_t(); }

const node_metaclass_t* color_match_node_t::metaclass() const
{
    return &color_match_node_metaclass();
}
...
static bool registered = node_factory_t::instance().register_node(
    color_match_node_t::color_match_node_metaclass());
