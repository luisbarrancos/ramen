...
void cm_calc_mean_and_stddev(const image::const_image_view_t& img,
                             Imath::V3f&                      mean,
                             Imath::V3f&                      std)
{
    using namespace boost::gil;
    ...
    std = Imath::V3f(
        std::sqrt((double) var.x), std::sqrt((double) var.y), std::sqrt((double) var.z));
}

struct transfer_color_fun
{
    transfer_color_fun(const Imath::V3f& src_mean,
...
void color_match_node_t::do_process(const render::context_t& context)
{
    ...
    // process input 1
    {
        image::const_image_view_t src_view(
            input_as<image_node_t>(1)->const_subimage_view(input_as<image_node_t>(1)->format()));
    ...
    {
        image::const_image_view_t src_view(
            input_as<image_node_t>(2)->const_subimage_view(input_as<image_node_t>(2)->format()));
        image::image_t tmp(src_view.width(), src_view.height());
        boost::gil::tbb_transform_pixels(src_view, boost::gil::view(tmp), convert_rgb_to_lab_fun());
        cm_calc_mean_and_stddev(boost::gil::const_view(tmp), src_mean, src_dev);
    }

    boost::gil::tbb_transform_pixels(input_as<image_node_t>(0)->const_subimage_view(area),
                                     subimage_view(area),
    ...
    boost::gil::tbb_transform_pixels(const_subimage_view(area),
                                     subimage_view(area),
                                     transfer_color_fun(src_mean, src_dev, dst_mean, dst_dev));
    boost::gil::tbb_transform_pixels(
        const_subimage_view(area), subimage_view(area), convert_lab_to_rgb_fun());
}
...
static bool registered
    = node_factory_t::instance().register_node(color_match_node_t::color_match_node_metaclass());
