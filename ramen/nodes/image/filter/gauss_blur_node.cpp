...
namespace image
{
gauss_blur_node_t::gauss_blur_node_t()
: base_blur_node_t()
{
    set_name("gauss_blur");
}
...
void gauss_blur_node_t::do_process(const render::context_t& context)
{
    ...
    switch (channels)
    {
        case channels_rgba:
        {
            ...
            image::gaussian_blur_rgba(
                const_image_view(), tmp.rgba_view(), image_view(), stddev.x, stddev.y);
        }
        break;
...
const node_metaclass_t* gauss_blur_node_t::metaclass() const
{
    return &gauss_blur_node_metaclass();
}
...
static bool registered
    = node_factory_t::instance().register_node(gauss_blur_node_t::gauss_blur_node_metaclass());
