...
void checkerboard_node_t::do_process(const render::context_t& context)
{
    ...
    Imath::Box2f area(defined().min - translate, defined().max - translate);
...
const node_metaclass_t* checkerboard_node_t::metaclass() const
{
    return &checkerboard_node_metaclass();
}
