...
set_matte_node_t::set_matte_node_t()
: image_node_t()
{
    set_name("set_matte");
...
void set_matte_node_t::do_calc_bounds(const render::context_t& context)
{
    if (get_value<bool>(param("premultiply")))
        set_bounds(ImathExt::intersect(input_as<image_node_t>(0)->bounds(),
...
void set_matte_node_t::do_process(const render::context_t& context)
{
    ...
    boost::gil::tbb_transform_pixels(input_as<image_node_t>(0)->const_subimage_view(defined()),
                                     image_view(),
...
const node_metaclass_t& set_matte_node_t::set_matte_node_metaclass()
{
    ...
    if (!inited)
    {
        ...
        m.help
            = "Replaces the alpha channel of the first input by the alpha channel of the second input.";
...
static bool registered
    = node_factory_t::instance().register_node(set_matte_node_t::set_matte_node_metaclass());
