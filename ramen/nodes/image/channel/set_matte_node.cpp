...
void set_matte_node_t::do_process(const render::context_t& context)
{
    ...
    boost::gil::tbb_transform_pixels(
        input_as<image_node_t>(0)->const_subimage_view(defined()),
        image_view(),
        copy_rgb_and_clear_alpha());

    Imath::Box2i area =
        ImathExt::intersect(defined(), input_as<image_node_t>(1)->defined());
...
const node_metaclass_t* set_matte_node_t::metaclass() const
{
    return &set_matte_node_metaclass();
}
...
const node_metaclass_t& set_matte_node_t::set_matte_node_metaclass()
{
    ...
    if (!inited)
    {
        ...
        m.help = "Replaces the alpha channel of the first input by the alpha "
...
static bool registered = node_factory_t::instance().register_node(
    set_matte_node_t::set_matte_node_metaclass());
