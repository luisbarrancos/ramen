...
class RAMEN_API xform2d_node_t : public xform_node_t
{
  ...
  protected:
    ...
    xform2d_node_t(const xform2d_node_t& other)
      : xform_node_t(other)
    {
    }
    void operator=(const xform2d_node_t&);

    matrix3_type global_matrix_at_frame(
        float                                     frame,
        const std::vector<const xform2d_node_t*>& xforms,
        int                                       subsample = 1) const;

    bool quad_is_convex(const std::array<vector2_type, 4>& quad) const;

    Imath::Box2i transform_box(
  ...
  private:
    virtual matrix3_type calc_transform_matrix_at_frame(
        float frame,
        int   subsample = 1) const;
    virtual matrix3_type do_calc_transform_matrix_at_frame(
        float frame,
        int   subsample = 1) const = 0;
