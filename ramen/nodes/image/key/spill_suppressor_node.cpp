                                         ...
                                         const render::context_t&         context)
{
    ...
    {
        int channel_mask = 0;
        if (get_value<bool>(param("dred")))
            channel_mask |= (1 << 0);
        if (get_value<bool>(param("dgreen")))
            channel_mask |= (1 << 1);
        if (get_value<bool>(param("dblue")))
            channel_mask |= (1 << 2);
        if (get_value<bool>(param("dcyan")))
            channel_mask |= (1 << 3);
        if (get_value<bool>(param("dmagenta")))
            channel_mask |= (1 << 4);
...
const node_metaclass_t* spill_suppressor_node_t::metaclass() const
{
