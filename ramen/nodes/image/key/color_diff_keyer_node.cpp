...
{
    color_diff_key_fun(const Imath::Color4f& key,
                       float                 screen,
                       float                 complement,
                       float                 shadows,
                       float                 alpha_low,
...
void color_diff_keyer_node_t::do_create_params()
{
    ...
    r->set_id("output");
    r->menu_items()
        = std::vector<std::string>({ "Premultiplied", "Unpremultiplied", "Alpha", "Status" });
    add_param(r);
...
void color_diff_keyer_node_t::do_process(const render::context_t& context)
{
    ...
    else
    {
        ...
        Imath::Box2i area(
            ImathExt::intersect(ImathExt::intersect(defined(), in0->defined()), in1->defined()));
