...
private:
    ...
    void do_mouse_enter_event(const ui::mouse_enter_event_t& event) override;
    void do_mouse_leave_event(const ui::mouse_leave_event_t& event) override;

    bool do_mouse_press_event(const ui::mouse_press_event_t& event) override;
    void do_mouse_move_event(const ui::mouse_move_event_t& event) override;
...
public:
    static void draw_shape(const shape_t&           s,
                           ...
                           float                    node_aspect,
                           bool                     axes = true);
    static void draw_axes(const shape_t&           s,
                          const ui::paint_event_t& event,
                          const Imath::Color3c&    col,
                          float                    node_aspect);
    static void draw_control_polygon(const shape_t& s, float pixel_scale);
    static void draw_triple(const triple_t&    t,
                            const Imath::M33f& m,
                            const Imath::V2f&  shape_offset,
                            float              pixel_scale);

    static void draw_bezier_span(const bezier::curve_t<Imath::V2f>& c,
                                 const Imath::M33f&                 m,
                                 const Imath::V2f&                  shape_offset);
    static void draw_bezier_curve(const Imath::V2f& a,
                                  const Imath::V2f& b,
                                  const Imath::V2f& c,
                                  const Imath::V2f& d);
};
