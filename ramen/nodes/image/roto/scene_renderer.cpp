...
public:
    ...
    boost::gil::gray32f_pixel_t operator()(const boost::gil::gray32f_pixel_t& fg,
...
scene_renderer_t::scene_renderer_t(const scene_t&      scene,
...
: scene_(scene)
{
    ...
    for (const shape_t& s : scene_)
    {
        ...
        box.min.y
            -= std::ceil((double) g / subsample_) + std::ceil((double) blur.y / subsample_) + 1;
...
void scene_renderer_t::render_and_filter_shape(const shape_t& s, int bbox_index)
{
    ...
    if (g != 0.0f)
    {
        ...
        image::dilate( subbuf_view, boost::gil::view( tmp_), subbuf_view, g / aspect_ / subsample_,
        g / subsample_);
        */
        boost::gil::fill_pixels(boost::gil::view(tmp_), image::gray_pixel_t(0));
        image::dilate(
            buf_view, boost::gil::view(tmp_), buf_view, g / aspect_ / subsample_, g / subsample_);
    }
    ...
    if (!common_area.isEmpty())
    {
        ...
        boost::gil::tbb_transform2_pixels(
            boost::gil::subimage_view(buf_view,
                                       ...
                                       int                subsample) const
{
    ...
    // last segment
    p2 = transform_point(
        s.triples()[s.triples().size() - 1].p2(), shape_offset, m, subsample, offset);
...
Imath::V2f scene_renderer_t::transform_point(const Imath::V2d&  p,
