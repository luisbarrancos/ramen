...
{
  public:
    composite_layer(float c, float o)
      : color_(c)
      , opacity_(o)
    {
    }

    boost::gil::gray32f_pixel_t operator()(
        const boost::gil::gray32f_pixel_t& fg,
        const boost::gil::gray32f_pixel_t& bg) const
    {
        float a = fg[0] * opacity_;
        float a_inv = 1.0f - a;
        return boost::gil::gray32f_pixel_t(
            clamp((color_ * a) + (bg[0] * a_inv), 0.0f, 1.0f));
    }
...
{
    assert(bbox_index >= 0 && bbox_index <= filtered_bboxes_.size());
    ...
    {
        /*
        image::gray_image_view_t subbuf_view( boost::gil::subimage_view(
        buf_view, bboxes_[bbox_index].min.x -
        filtered_bboxes_[bbox_index].min.x, bboxes_[bbox_index].min.y -
        filtered_bboxes_[bbox_index].min.y, bboxes_[bbox_index].size().x + 1,
        bboxes_[bbox_index].size().y + 1));

        image::dilate( subbuf_view, boost::gil::view( tmp_), subbuf_view, g /
        aspect_ / subsample_, g / subsample_);
        */
        boost::gil::fill_pixels(boost::gil::view(tmp_), image::gray_pixel_t(0));
        image::dilate(
            buf_view,
    ...
    if (blur.x != 0.0f || blur.y != 0.0f)
    {
        ...
        image::box_blur_gray(
            ...
            boost::gil::view(tmp_),
            buf_view,
            blur.x / aspect_ / subsample_,
            blur.y / subsample_,
            1);
    }

    Imath::Box2i common_area =
        ImathExt::intersect(area_, filtered_bboxes_[bbox_index]);

    if (!common_area.isEmpty())
    {
        image::gray_image_view_t bg_view(boost::gil::subimage_view(
            boost::gil::view(pixels_),
        ...
        boost::gil::tbb_transform2_pixels(
            ...
            bg_view,
            bg_view,
            composite_layer(s.color(), s.opacity()));
    }
}

void scene_renderer_t::convert_to_path(
    const shape_t&     s,
    ...
    int                subsample) const
{
    ...
    p0 = transform_point(
        s.triples()[0].p1(), shape_offset, m, subsample, offset);
    path.move_to(p0.x, p0.y);

    for (int i = 0; i < s.triples().size() - 1; ++i)
    {
        p2 = transform_point(
            s.triples()[i].p2(), shape_offset, m, subsample, offset);
        p0 = transform_point(
            s.triples()[i + 1].p0(), shape_offset, m, subsample, offset);
        p1 = transform_point(
            s.triples()[i + 1].p1(), shape_offset, m, subsample, offset);
        path.curve4(p2.x, p2.y, p0.x, p0.y, p1.x, p1.y);
    }

    // last segment
    p2 = transform_point(
        s.triples()[s.triples().size() - 1].p2(),
        shape_offset,
        m,
        subsample,
        offset);
    p0 = transform_point(
        s.triples()[0].p0(), shape_offset, m, subsample, offset);
    p1 = transform_point(
        s.triples()[0].p1(), shape_offset, m, subsample, offset);
    path.curve4(p2.x, p2.y, p0.x, p0.y, p1.x, p1.y);
    path.close_polygon();
...
Imath::Box2f scene_renderer_t::calc_bbox(const shape_t& s, int subsample)
{
    ...
    agg::bounding_rect_single(
        cpath, 0, &(box.min.x), &(box.min.y), &(box.max.x), &(box.max.y));
    return box;
}

Imath::V2f scene_renderer_t::transform_point(
    const Imath::V2d&  p,
