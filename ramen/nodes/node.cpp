...
bool node_t::interacting() const { return flags_ & interacting_bit; }
...
{
    assert(hash_generator().empty());
    assert(!hash_generator().finalized());
...
{
    assert(metaclass() && "Trying to serialize an abstract node");
    out.begin_map();
    write_node_info(out);
    param_set().write(out);
    do_write(out);
