...
namespace ramen
{
transform2_param_t::transform2_param_t(const std::string& name, const std::string& id)
...
transform2_param_t::matrix_type transform2_param_t::matrix_at_frame(float frame,
                                                                    float aspect,
                                                                    int   subsample) const
{
    ...
    transform2_param_t::matrix_type m
        = matrix_type().setTranslation(-c) * matrix_type().setScale(Imath::V2d(aspect, 1.0))
          * matrix_type().setScale(s)
          * matrix_type().setRotation(angle * math::constants<double>::deg2rad())
          * matrix_type().setTranslation(t) * matrix_type().setScale(Imath::V2d(1.0 / aspect, 1.0))
          * matrix_type().setTranslation(c);

    return m;
}

transform2_param_t::matrix_type transform2_param_t::xform_blur_matrix_at_frame(float frame,
                                                                               float t,
                                                                               float aspect,
                                                                               int subsample) const
{
    ...
    transform2_param_t::matrix_type m
        = transform2_param_t::matrix_type().setScale(Imath::V2d(aspect, 1.0))
          ...
          * transform2_param_t::matrix_type().setRotation(angle
                                                          * math::constants<double>::deg2rad())
