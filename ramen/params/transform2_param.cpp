...
transform2_param_t::transform2_param_t(const transform2_param_t& other)
...
const param_t& transform2_param_t::translate_param() const
{
    return params()[1];
}
...
transform2_param_t::matrix_type transform2_param_t::matrix_at_frame(
    float frame,
    float aspect,
    int   subsample) const
{
    Imath::V2f c =
        get_absolute_value_at_frame<Imath::V2f>(center_param(), frame) /
        subsample;
    Imath::V2f t =
        get_value_at_frame<Imath::V2f>(translate_param(), frame) / subsample;
    float      angle = get_value_at_frame<float>(rotate_param(), frame);
    Imath::V2f s = get_value_at_frame<Imath::V2f>(scale_param(), frame);

    transform2_param_t::matrix_type m =
...
transform2_param_t::matrix_type transform2_param_t::xform_blur_matrix_at_frame(
    float frame,
    float t,
    float aspect,
    int   subsample) const
{
    ...
    transform2_param_t::matrix_type m =
        transform2_param_t::matrix_type().setScale(Imath::V2d(aspect, 1.0)) *
        transform2_param_t::matrix_type().setTranslation(-c) *
        transform2_param_t::matrix_type().setScale(s) *
        transform2_param_t::matrix_type().setRotation(
            angle * math::constants<double>::deg2rad()) *
        transform2_param_t::matrix_type().setTranslation(c + d) *
        transform2_param_t::matrix_type().setScale(
            Imath::V2d(1.0 / aspect, 1.0));

    return m;
}

/*
void transform2_param_t::apply_track( float start_frame, float end_frame, const
image::tracker_node_t *tracker, image::apply_track_mode mode,
image::apply_track_use use)
{

    Imath::V2f center = get_absolute_value_at_frame<Imath::V2f>( center_param(),
start_frame); float2_param_t *trans = dynamic_cast<float2_param_t*>(
&translate_param()); float_param_t *rot = dynamic_cast<float_param_t*>(
&rotate_param()); float2_param_t *scale = dynamic_cast<float2_param_t*>(
&scale_param());

    param_set()->begin_edit();
    tracker->apply_track( start_frame, end_frame, mode, use, center, trans, rot,
scale); param_set()->end_edit( true); app().ui()->update_anim_editors();
}
*/
