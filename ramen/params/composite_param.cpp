...
composite_param_t::composite_param_t( const core::string8_t& name) : param_t( name), create_track_( true) {}
...
void composite_param_t::do_add_param( param_t *p)
{
    ...
    if( param_set())
    {
        ...
        {
            param_set()->find( p->id());
            throw std::runtime_error( core::string8_t( "Duplicated param id found: ").append( p->id().c_str()));
        }
