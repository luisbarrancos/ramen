...
namespace ramen
{
curve_param_t::curve_param_t(const std::string& name)
  : animated_param_t(name)
{
    private_init();
}

curve_param_t::curve_param_t(const curve_param_t& other)
...
void curve_param_t::do_add_to_hash(hash::generator_t& hash_gen) const
{
    hash_gen << curve().str();
}
